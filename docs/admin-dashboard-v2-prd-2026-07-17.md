...
Every query in the window, grouped by normalized query, library filter, and query origin. Searchers, channels, countries, and tools are measures or details within the group. Columns: query (with library, origin, and tool subtext), requests, estimated client IDs, outcome, country, channel, typical result, and last seen. Estimated client IDs are not treated as people. The typical result is the median recorded result count and is unavailable when a row mixes incompatible result units. The free-text search supports plain text plus key:value filters (zero:true, low:true, venue:web, country:DE, origin:user, registered:true). The three downloads are Search summary, Request log, and Audit bundle. The legacy evidence tables are deleted.
