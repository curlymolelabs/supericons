...
- Prerequisite investigation, first: the 14-case verification found the query frame reporting `unclassified`, empty concepts, and `matched: false` for every tested English query in the beta package, including queries that succeeded. Determine whether the packaged intent-graph and query-frame data fails to load in the local package or whether the diagnostics misreport. If the intent data is not loading locally, fixing the packaging may recover phrase quality before any new wiring; the finding changes what batch 2 builds on, so it is resolved before the variant work starts.
...
3. One-call contract shell per `one-call-contract-2026-07-19.md`: `suggested_response_markdown` in every `search_icons` response, with `image_url` and `markdown_image` only when usable icons exist; tool-description steering; server `instructions` at initialize stating the display rule; `preview_icons` graceful truncation (accept long lists, truncate to 12, report `truncated_from`); forgiving input coercion instead of bare protocol errors.
