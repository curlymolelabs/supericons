...
| `FR-46` | `recommend_icons` accepts up to 20 UI slots in one call. Correctable recommendation and preview input problems return a stable code, a plain-language reason, and a useful next step instead of a bare protocol error. Recommendation service failures preserve safe retry and limit details. Preview requests may ask for more than 12 icons; inline output is clamped to 12 with a warning while the browser preview retains up to 24 accepted refs. | Agent usability, reliability, `FR-24`, `FR-34`, `D-031` | Local and hosted MCP integration fixtures accept 20 slots, explain a 21-slot request, preserve timeout and 429 recovery details, use one grouped request within the 96-query cap, and turn a 13-icon preview request into a successful 12-icon inline response with all accepted refs in the browser link. |
