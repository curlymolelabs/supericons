...
| `FR-49` | The stable npm package, Railway hosted MCP, and public web search use the same packaged deterministic Search v2 engine data and ranking code. Stable npm runs both search and recommendation locally by default. Railway runs both tools locally by default and retains one controlled hosted fallback for engine exceptions. The web sends search inputs to a browser-safe Railway endpoint that returns ranked public icon IDs without SVG bytes or protected ranking data. Every surface reports its execution route, and the stable package version, Railway health version, website release record, and public docs identify the same release. | `G-03`, `G-07`, `G-09`, `FR-24`, `FR-41`, `FR-48`, `D-033` | The fixed 225-case fingerprint, clean-installed stdio route fingerprint, real HTTP MCP tests, browser-safe response checks, English and non-English browser interaction tests, public-boundary probes, exact package inspection, and live post-release checks pass. Rollback controls can independently restore the prior Railway deployment, Netlify deploy, and npm `latest` tag. |
...
- A synchronized deterministic release may update Railway, web, and npm in one bounded sequence only when each surface passes its own gate and keeps an independent rollback target.
