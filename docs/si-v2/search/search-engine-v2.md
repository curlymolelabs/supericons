...
| `FR-46` | `recommend_icons` accepts up to 20 UI slots in one call. It deduplicates identical generated searches, uses an additive grouped endpoint, and retries each distinct search individually once when grouped mode is unavailable, invalid, or returns invalid JSON. Grouped failures use separate resilience state and cannot block the stable fallback. A configured custom individual endpoint stays individual unless a grouped endpoint is explicitly configured. Individual and grouped results use the same local fallback, while hosted 4xx responses remain errors instead of being hidden. Correctable recommendation and preview input problems return a stable code, a plain-language reason, and a useful next step. Preview requests may ask for more than 12 icons; inline output is clamped to 12 with a warning while the browser preview retains up to 24 accepted refs. Grouped mode fails closed if tier enforcement is enabled before race-safe accounting is ready. | Agent usability, reliability, `FR-24`, `FR-34`, `FR-43`, `D-031` | Local and hosted integration fixtures accept 20 slots, explain a 21-slot request, preserve timeout and 429 details, prove empty-result local fallback, reject malformed grouped responses, retry successful non-JSON responses, prove concurrent grouped failures cannot block stable fallback, preserve custom routing, prevent grouped allowance bypass, deduplicate repeated slots, and turn a 13-icon preview request into a successful 12-icon inline response. |
| `FR-47` | A recommendation prerelease must complete before the 20-second client timeout. Actual routed end-to-end p95 must be at most 3 seconds for 1 slot, 10 seconds for 10 slots, and 15 seconds for 20 slots, with zero client timeouts in the approved workload. Every sample records whether the server reports a first request or a reused worker. | Agent reliability, `FR-35`, `FR-38`, `D-031` | The exact candidate bytes pass public-safe measurements for 1, 10, and 20 resolved slots, including one supported non-English 20-slot case. All routed samples count even when the platform provides no worker reuse. Any miss blocks publication. |
| `FR-48` | The Railway hosted MCP service runs `recommend_icons` from its in-process deterministic index by default. Successful and honest no-result calls make zero Supabase search requests. A local engine exception may make at most one stable hosted search request for the whole recommendation call. The public response and best-effort telemetry identify the execution path. Local execution is not charged against hosted search allowances. | `G-03`, `G-07`, `G-10`, `FR-24`, `FR-43`, `D-032` | The real HTTP MCP server passes 1, 10, and 20-slot English checks, one supported non-English 20-slot check, clarification and quality checks, a zero-hosted-call assertion, one-call fallback injection, telemetry attribution, fresh-call latency limits, repeated-call p95 below 500 ms, and rollback-switch verification. |
| `FR-49` | Search surfaces preserve their accepted public behavior contracts and shared-snapshot compatibility, but release independently according to actual impact. A surface changes only when required code, data, assets, content, configuration, dependencies, security posture, platform compatibility, runtime behavior, or public contract changes for that surface. A shared source-file change alone does not make every surface affected. Matching version labels, release dates, generated timestamps, or source revision labels are not reasons to deploy. Release records map the exact source, artifact, deployed version, and rollback target for every affected surface. Hosted routing remains governed by `FR-52`. An unchanged MCP URL, tool schema, authentication model, permissions set, and public capability set does not require app or directory resubmission. | `G-03`, `G-07`, `G-09`, `FR-24`, `FR-41`, `FR-48`, `FR-52`, `D-033`, `D-046` | A release impact record names every affected and unaffected surface and traces each mutation to the changed call path or consumed artifact. Each affected exact artifact passes its applicable package, HTTP, browser, public-boundary, and live checks and keeps an independent rollback target. Shared behavior changes pass decision-parity checks against a pinned common snapshot. Internal package metadata remains coherent, but no gate requires unrelated live surfaces to report the same version. |
| `FR-50` | Common words may resolve through a reviewed meaning group when they do not name an icon literally. The same deterministic meaning groups support English and the maintained locales. Exact identities and technical terms keep priority, word boundaries prevent accidental substring matches, and unsupported text returns an honest empty result. Language expansion follows measured traffic and zero-result evidence, then remains protected by locale-specific fixtures. | `G-01`, `G-02`, `G-03`, `G-07`, `FR-12`, `FR-21`, `FR-23`, `FR-24`, `D-034` | The exact release bytes pass the fixed 225-case suite, at least 244 English meaning checks, at least 612 localized meaning checks across 11 maintained locales, 638 established multilingual fixtures, honest no-result fixtures, a cold-start observation, and local search p95 below 500 ms. |
| `FR-51` | After the synchronized 0.4.20 release, deterministic Search v2 operates in evidence-driven maintenance mode. Small meaning, ranking, localization, and metadata corrections must come from repeated reviewed gaps. The next bounded experiment may improve the structured zero-result response and measure same-session recovery, but it is not a release blocker. Embeddings, a query encoder, and request-time LLM or agent reformulation remain paused until a new accepted decision proves a persistent meaningful gap and defines privacy, latency, cost, abuse, evaluation, caching, and rollback boundaries. | `G-07`, `G-08`, `G-09`, `FR-12`, `FR-21`, `FR-31`, `FR-32`, `D-021`, `D-035` | The implementation ledger distinguishes released behavior from planned maintenance work. Any recovery experiment separates genuine top-level queries from internal or scripted traffic, measures recovery within two tool calls, reports client and locale segments and telemetry limits, reviews false relevance, and cannot enable semantic or model-assisted serving without a later accepted decision and independent release gate. |
| `FR-52` | Railway hosted `search_icons` and the public Railway search endpoint require a valid response from the established hosted variant engine. Packaged local retrieval may run at the same time to reduce latency, but it cannot answer before hosted success or hide a hosted dependency error. Valid hosted and local candidates may be fused with reviewed relevance rules. A valid hosted no-result may use the packaged result as `local_fallback`. `recommend_icons` remains local-first. Responses and telemetry distinguish `hosted`, `hosted_fused`, and `local_fallback`, and the public endpoint continues to omit SVG and protected semantic data. | `G-01`, `G-02`, `G-03`, `G-07`, `FR-16`, `FR-21`, `FR-24`, `FR-48`, `D-036`, `D-037` | The exact candidate and live deployment pass judged agent-style multiword relevance and forbidden-result checks, strict-library and multilingual cases, honest no-results, MCP-to-public-HTTP ordered-reference parity, browser-safe payload checks, agent-readable errors, preview and recommendation regressions, the fixed 225-case suite, forced hosted-error checks, and rollback verification. |
| `FR-53` | Release checks that call live search use an authenticated, time-bounded controlled-run marker. The server classifies a request as test traffic only after verifying the marker, and invalid or missing markers remain normal traffic. A hosted search repair that changes packaged search behavior uses a new immutable npm version, and the exact archive must pass clean-install parity before it can replace `latest`. | `G-03`, `G-08`, `G-09`, `FR-25`, `FR-41`, `FR-42`, `FR-45`, `D-037` | Tampered, expired, and unsigned controlled markers fail closed to normal classification. A signed live marker is recorded as test traffic. The exact package archive passes public-boundary checks, package inspection, clean installation, all 225 fixed cases, ordered route parity, and the same judged product matrix used by the hosted release. |
| `FR-54` | Search measurement records one final top-level outcome for each eligible website search episode or MCP tool call. `episode_id` identifies that product action, `attempt_id` identifies internal work, and `recovery_chain_id` may connect later product actions without collapsing them. The browser coordinator determines the final website result after local and hosted work settles, while a trusted server validates channel, environment, and traffic class before storage. Superseded and incomplete website episodes remain diagnostics. Internal attempts never enter search totals or the true-zero denominator. The final-outcome ledger is additive and does not change the `search_request_audit` allowance unit. Historical Web and stable Local MCP coverage remains marked incomplete before independent verified cutovers. The 150 ms input debounce, 2,500 ms countable-search idle interval, Search v2 behavior, website network behavior, tool responses, and result order remain unchanged. | Measurement honesty; `VC-3`, `VC-6`, `VC-7`, `FR-24`, `FR-44`, `FR-45`, `FR-53`, `D-029`, `D-030`, `D-035`, `D-038` | Additive migration and rollback checks pass. Website, Hosted MCP, and Local MCP fixtures produce one final outcome per eligible episode with linked diagnostics, distinct repeated calls, no superseded or incomplete zeros, server-controlled trusted classifications, unchanged product fingerprints, unchanged allowance rows and cost, matching dashboard and export totals, and visible Web and Local MCP cutover warnings. |
...
- A release changes only the surfaces shown to be affected. Version-label alignment alone never authorizes a Railway, website, npm, app, or directory mutation. Every changed surface keeps its own gate and rollback target.
