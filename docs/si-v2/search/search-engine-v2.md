...
| `FR-46` | `recommend_icons` accepts up to 20 UI slots in one call. It deduplicates identical generated searches, uses an additive grouped endpoint, and retries each distinct search individually once when grouped mode is unavailable or invalid. Individual and grouped results use the same local fallback. Correctable recommendation and preview input problems return a stable code, a plain-language reason, and a useful next step. Hosted limits remain errors instead of being hidden by fallback. Preview requests may ask for more than 12 icons; inline output is clamped to 12 with a warning while the browser preview retains up to 24 accepted refs. Grouped mode fails closed if tier enforcement is enabled before race-safe accounting is ready. | Agent usability, reliability, `FR-24`, `FR-34`, `FR-43`, `D-031` | Local and hosted integration fixtures accept 20 slots, explain a 21-slot request, preserve timeout and 429 details, prove empty-result local fallback, reject malformed grouped responses, prove grouped-endpoint rollback compatibility, prevent grouped allowance bypass, deduplicate repeated slots, and turn a 13-icon preview request into a successful 12-icon inline response. |
