...
The final live 28-case product matrix passed through both public HTTP and hosted MCP with identical ordered icon references. It covers the confirmed false zeros and weak rankings for hard hats, network graphs, connected people, disconnected links, tow trucks, cranes, forklifts, excavation vehicles, and other agent-style phrases. It also covers English meaning search, Spanish, Japanese, and Portuguese cases, strict library behavior, browser-safe responses, and honest no-results. Public HTTP latency measured 2,226.2 ms median, 4,051.6 ms p95, and 4,322.2 ms maximum, with no call above five seconds.

Forced outage checks prove that hosted network and server failures remain visible instead of being hidden by local results. Responses distinguish `hosted`, `hosted_fused`, and `local_fallback`. One authenticated release marker was recorded as test traffic on the active deployment. The exact 0.4.21 npm archive is prepared and verified but is not yet published. The incident proof is recorded in [`reviews/search-v2-incident-closure-2026-07-23.md`](reviews/search-v2-incident-closure-2026-07-23.md).
