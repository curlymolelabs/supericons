...
| `P1` Shared deterministic understanding | Latency measurement stopped on live parity; deterministic tie fix is local and awaits a new gate | Search intent core, generated rules/graph, shared policy-aware query-frame modules, recommendation clarification, maintained ranking policy, ambiguity diversification, all 50 approved SI brand classifications, rejected-alias enforcement, final-fusion policy enforcement, strict/prefer/all modes, additive audit contract, safe stage timing, approximate candidate payload measurement, additive SVG-free beta candidate RPC, final-result SVG hydration, deterministic final tie-break, guarded SQL runner, clarification-safe aggregation, locale-count helper, prerelease routing, and rollback plan | Treatment warm search p95 was 1,785.161 ms across 25 requests with zero errors, but live response parity failed on an unstable equal-score tie; the isolated endpoint was deleted before recommendation measurement; the local tie regression and 225-case checks now pass | The `@supericons/mcp` 0.4.18-beta.0 package dry-run and package checks verified 38 files; not published | Beta measurement schema and migrations `20260712` and `20260713150000` remain deployed; isolated beta function is deleted; production functions remain on versions 34 and 35 | Control and treatment search were observed live; recommendation was not measured; production checks returned HTTP 200 after rollback | `references/verification/search-v2-latency-gate-a-partial-execution-2026-07-13.md`; `references/verification/search-v2-deterministic-tie-fix-2026-07-13.md`; `references/verification/search-v2-latency-gate-a-preparation-2026-07-13.md`; `references/verification/search-v2-latency-phase1-local-2026-07-13.md`; `references/verification/search-v2-deterministic-mcp-beta-gate-c-rollback-2026-07-13.md` |
...
2. Review the partial execution and deterministic tie-fix evidence.
3. Build new immutable control and treatment commits, then issue a new fingerprint-bound approval request.
