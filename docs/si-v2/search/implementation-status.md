...
| `P1` Shared deterministic understanding | Round-trip reduction is locally verified; live remeasurement is pending | Search intent core, generated rules/graph, shared policy-aware query-frame modules, recommendation clarification, maintained ranking policy, ambiguity diversification, all 50 approved SI brand classifications, rejected-alias enforcement, final-fusion policy enforcement, strict/prefer/all modes, additive audit contract, safe stage timing with worker request order, additive SVG-free candidate RPCs, one-call ordered candidate retrieval, grouped recommendation search, logical-query rate-limit accounting, synchronous grouped audit writes, final-result SVG hydration, deterministic final tie-break, distinct family retrieval, isolated control/treatment endpoints, guarded SQL runners, and rollback plans | Clean parent and implementation worktrees both produce the 225-case fingerprint `e610fce3...`; unrelated dirty taxonomy and icon-index files explain the previously recorded working-tree hashes. Disposable PostgreSQL proves separate/batched candidate parity and rollback; full HTTP bytes match across five response cases; grouped recommendation result parity, clarification short-circuit, rate-limit cost, and synchronous audit checks pass. Prior live evidence still shows warm direct search at 1,645.140 ms and failed first-request limits. | The changed `@supericons/mcp` 0.4.18-beta.0 package dry-run contains 38 expected files; not published | Beta measurement schema and migrations `20260712` and `20260713150000` remain deployed; migration `20260714120000` and the new isolated functions are not deployed; production functions remain unchanged in the saved live record | Prior control and treatment parity and direct search were observed live; the new round-trip path has not been observed live | `references/verification/search-v2-roundtrip-reduction-local-2026-07-14.md`; `references/verification/search-v2-fingerprint-source-correction-2026-07-14.md`; `references/verification/search-v2-latency-rerun-execution-2026-07-14.md` |
...
Measure the locally verified round-trip reduction before another deterministic MCP beta:
...
2. Audit the local round-trip implementation and its guarded SQL runner.
3. Prepare one fingerprint-bound approval for migration `20260714120000`, one deployment per isolated endpoint, interleaved read-only measurements, and deletion of both endpoints.
4. Keep normal `db push`, historical renames, older ledger repair, production function deployment, and npm publication prohibited.
5. Use safety failures as stop conditions. Record performance misses and complete the approved diagnostic phases before deleting the isolated endpoints.
