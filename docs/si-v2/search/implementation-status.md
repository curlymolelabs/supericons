...
| `P1` Shared deterministic understanding | Current main contains the reviewed Search v2 and Material integration. Search-only beta `0.4.19-beta.0` failed live Gate C, was rolled back, and was not published. | Search intent and ranking policy; expressive related-result ordering; recommendation clarification; strict/prefer/all modes; legal English and localized workload limits; end-to-end tool latency logging; persisted worker evidence; SVG-free ordered candidate RPCs; shared recommendation candidate, metadata, semantic, SVG, and bulk-audit work; logical-query rate-limit accounting; public-safe in-band timing; Material ranking and fixed-preset SVG hydration; usage-event dedupe; guarded SQL runners; rollback plans; incident guardrails; inactive monitoring drafts | The combined Search v2 and Material gates pass through main integration commit `ed3dcdea7`. The clean 225-case fingerprint is `ef293409...a76c8`. Local beta gates pass, but live Gate C failed with 7,151.057 ms search p95, 2,286.601 ms localized p95, and one error among 38 captured hosted requests. All six error-or-over-2,000-ms audit rows were first requests on new workers. The dominant internal stage remains unverified because the failed run did not retain the safe timing payload. The corrected local harness now retains future stage, error, localized-attempt, and worker-group evidence, but it has not been live-verified. | A clean package at implementation commit `415f401b7` contains 44 files and reproduces tarball SHA-256 `6bb6bda4...65c1c`. Clean-install checks pass. The package was not published. | Migration `20260714180000` is deployed and recorded. The isolated beta endpoint was deployed once and deleted after Gate C failed. Read-only closeout confirms `mcp-search` version 38 and `search-icons` version 35 remained active. Migration `20260714190000` remains local and excluded. | Railway health was previously observed live at MCP version `0.4.18` with 8,524 Material assets. The isolated Search v2 beta was observed live but failed its latency and error gates, then was deleted. | `references/verification/search-v2-main-reintegration-2026-07-16.md`; `references/verification/search-v2-search-only-beta-0.4.19-preparation-2026-07-16.md`; `references/verification/search-v2-search-only-beta-gate-c-execution-2026-07-16.md`; `references/verification/search-v2-gate-c-harness-correction-2026-07-16.md`; `references/verification/material-railway-recovery-narrow-completion-2026-07-16.md` |
...
| Search v2 beta | Manifest `bf59e6cf...1d734` was executed. Gate C failed, the endpoint was deleted, and `0.4.19-beta.0` was not published. npm `latest` remains `0.4.17`. The later local Gate C corrections retain timed error evidence and strictly bind finalization to the full 41-request workload, typed manifest and live evidence, worker groups, time window, and release gates. They do not authorize another live attempt. |
...
1. Independently review the workload-completeness correction recorded in `references/verification/search-v2-gate-c-workload-completeness-correction-2026-07-16.md`.
