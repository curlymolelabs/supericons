...
Railway deployment `77976dd0-2147-4bb5-bc01-e0b9896cffb6` is live with image digest `sha256:d66f7aa1421c79ddae9294327b749536dfc7c08ddee8c3ad0d8891af47603b59`. Live health reports version 0.4.22, hosted-primary search, local-first recommendations, and closed search circuits with zero consecutive failures. The MCP URL and registered tool schemas did not change, so existing hosted users and submitted channel configurations need no update.

The final signed 39-case product matrix passed through both public HTTP and hosted MCP with identical ordered icon references. It covers the confirmed false zeros and weak rankings for hard hats, network graphs, connected people, disconnected links, tow trucks, cranes, forklifts, excavation vehicles, and other agent-style phrases. It also covers strict library behavior, browser-safe responses, honest no-results, and all 11 maintained non-English locales: Simplified Chinese, Traditional Chinese, Japanese, Korean, Spanish, Portuguese, German, Arabic, Hindi, Vietnamese, and Thai. Live latency varied by call and remains a monitoring target.

Forced outage checks prove that hosted network and server failures remain visible instead of being hidden by local results. Responses distinguish `hosted`, `hosted_fused`, and `local_fallback`. Signed release checks are stored as test traffic instead of organic traffic. npm 0.4.22 is published as `latest`, and a fresh registry download is byte-identical to the approved archive. Its clean-installed stdio route passed all 225 maintained cases. The incident proof is recorded in [`reviews/search-v2-incident-closure-2026-07-23.md`](reviews/search-v2-incident-closure-2026-07-23.md).
...
Railway deployment `ee02bdca-96b6-46f2-9637-7636a2271f33` was the verified pre-repair baseline with future returned icon references, build and execution metadata, traffic classification, root request linking, and structured `icon_not_found` errors. Those capabilities remain in the current deployment. At that baseline, health reported version 0.4.20, 8,524 Material assets, and a closed hosted-search resilience circuit with zero active or queued calls.
...
Migration `20260722230000_admin_quality_controlled_traffic_correction.sql` corrected 3,041 high-confidence validation rows in production without deleting history. A fresh seven-day database aggregate then measured 879 unclassified hosted direct searches with 108 zero results and one error, plus 267 unclassified recommendations with five zero results and two errors. Exact lookup now reports 541 found, 26 not found, and five errors across 572 attempts. A complete 11,014-event post-migration replay passed the scorecard's operational-count trust gate. The exact production data path generated its first page in 1.881 seconds and served the remaining complete 111-page export from the same snapshot in 1.638 seconds. Production browser authentication with the owner's current rotated secret remains an owner-operated smoke check because this session did not read that credential.

## Broad meaning coverage release

Version 0.4.20 introduced the `D-034` broad meaning release. Version 0.4.22 retains that coverage and adds the reviewed multiword, regional-locale, segmentation, and multilingual corrections required by `D-037`. Its fixed 225-case fingerprint is `3ec9fae...7d24`, and its clean-installed stdio fingerprint is `5bc36ea9...b711`. The earlier incident fingerprint changes are reviewed in `reviews/search-v2-incident-closure-fingerprint-review-2026-07-23.md`; the final all-locale evidence is recorded in the incident closure record.

The current Railway deployment retains the meaning coverage and adds the incident corrections. npm `latest` is 0.4.22. Netlify deploy `6a61dbbb3cfc0470a11de105` is live with the production search URL set to `https://mcp.supericons.dev/search-icons`. A headed browser check passed direct search phrases in all 11 maintained non-English locales, confirmed search icons among the first five displayed results, and reported zero console errors and zero console warnings. The unsupported query `florblequux` remains an honest no-result. The live web MCP guide continues to show keyless hosted setup, current local setup, and all three setup videos.

## Search maintenance state

`D-035` places the released deterministic engine in evidence-driven maintenance mode. Embeddings, a query encoder, and an internal LLM or agent fallback remain paused. The existing MCP no-result response already returns a stable code, plain-language hint, suggested response, and next step. Suggested visual queries, a recommended-tool field, a single-retry field, and same-session recovery reporting are planned as one bounded maintenance experiment when scheduled; they are not implemented and are not release requirements for 0.4.20. Normal product development returns to the controlling SI v2 roadmap.

## Previous synchronized stable release

Source revision `5a701234a9dfac2ea1145d4b3221c03ed1bbf43e` implemented the earlier `D-033` alignment as version 0.4.19. Version 0.4.20 now supersedes it on every public surface. The table below preserves the final 0.4.19 evidence as rollback history; its former remaining-boundary text is historical rather than current release status.

| surface | verified 0.4.19 behavior | historical boundary at capture |
| --- | --- | --- |
...
| Railway hosted MCP | Deployment `77976dd0-2147-4bb5-bc01-e0b9896cffb6` is live. Health reports MCP version 0.4.22, 8,524 Material assets, hosted-primary search, local-first recommendations, and closed search circuits. |
| npm registry | `latest` is 0.4.22 and `beta` is 0.4.19-beta.2. The 0.4.22 registry shasum is `7483202b5c63b03eedbe640c5df142d799bb155d`, and a fresh registry download is byte-identical to the approved archive SHA-256 `00fcb485...d6a9`. |
| Web grid | Netlify production deploy `6a61dbbb3cfc0470a11de105` is live at `supericons.dev`. A headed browser check passed all 11 maintained non-English locale searches with zero console errors and zero console warnings. The website called `mcp.supericons.dev/search-icons` successfully. |
...
| Search v2 stable | Version 0.4.22 is public under npm tag `latest` and is active on Railway. The hosted and local package surfaces passed the maintained fixed suite, the 39-case live product matrix, and all-locale browser smoke tests. The unchanged public MCP URL and unchanged tool schemas mean existing hosted users need no configuration update or channel resubmission. |
...
6. Preserve independent npm, Railway, and Netlify rollback targets for later Search v2 maintenance releases.
