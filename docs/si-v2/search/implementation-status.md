...
Railway deployment `02d329dc-66dd-4c20-9fa2-937246e24b9d` is live from source `b49039f97606e5a1e130c6a15bc7e6c6d6c1ea77`. Its image digest is `sha256:211e4fdc1d32dff5b094b334da340b1c5a394afd96509fceb924c64946ee0384`. Live health reports version 0.4.21, hosted-primary search, local-first recommendations, and a closed hosted-search circuit with zero consecutive failures. The MCP URL, website, and submitted ChatGPT app configuration did not change, so existing hosted users need no update.

The final post-publication 28-case product matrix passed through both public HTTP and hosted MCP with identical ordered icon references. It covers the confirmed false zeros and weak rankings for hard hats, network graphs, connected people, disconnected links, tow trucks, cranes, forklifts, excavation vehicles, and other agent-style phrases. It also covers English meaning search, Spanish, Japanese, and Portuguese cases, strict library behavior, browser-safe responses, and honest no-results. Public HTTP latency measured 2,146.2 ms median, 3,686.6 ms p95, and 3,862.9 ms maximum, with no call above four seconds.

Forced outage checks prove that hosted network and server failures remain visible instead of being hidden by local results. Responses distinguish `hosted`, `hosted_fused`, and `local_fallback`. One authenticated release marker was recorded as test traffic on the active deployment. npm 0.4.21 is published as `latest`, and a fresh registry download is byte-identical to the approved archive. Its clean-installed stdio route passed all 225 maintained cases. The incident proof is recorded in [`reviews/search-v2-incident-closure-2026-07-23.md`](reviews/search-v2-incident-closure-2026-07-23.md).
...
Version 0.4.20 introduced the `D-034` broad meaning release. The 0.4.21 incident candidate retains that coverage and adds the reviewed multiword and localized corrections required by `D-037`. Its fixed 225-case fingerprint is `4ee5e16c...37ad`, and its clean-installed stdio fingerprint is `c97a3c39...0e97`. All 22 fixed-suite changes are reviewed in `reviews/search-v2-incident-closure-fingerprint-review-2026-07-23.md`.

The current Railway deployment retains the meaning coverage and adds the incident corrections. npm `latest` is 0.4.21. Netlify deploy `6a5fe089d08a1cac5e8459a5` remains live because the browser search path was already healthy and required no incident deployment. Production searches for English `amazing` and `sports`, Japanese `スポーツ`, and Spanish `deportes` return reviewed results. The unsupported query `florblequux` returns no result. A headed browser check also confirmed English, Spanish, and Japanese multiword searches, the honest no-result state, successful calls to the live hosted search endpoint, and no console errors. The live web MCP guide continues to show keyless hosted setup, current local setup, and all three setup videos.
...
| Railway hosted MCP | Deployment `02d329dc-66dd-4c20-9fa2-937246e24b9d` is live. Health reports MCP version 0.4.21, 8,524 Material assets, hosted-primary search, local-first recommendations, and a closed hosted-search circuit. |
| npm registry | `latest` is 0.4.21 and `beta` is 0.4.19-beta.2. The 0.4.21 registry shasum is `75e4092cb930ac13e23056a135a069bb5e5272ec`, and a fresh registry download is byte-identical to the approved archive SHA-256 `e63248c2...e4d8`. |
| Web grid | Netlify production deploy `6a5fe089d08a1cac5e8459a5` is live at `supericons.dev`. A headed browser check passed English, Spanish, Japanese, and honest no-result searches with zero console errors. The website called the live `mcp.supericons.dev/search-icons` endpoint successfully. |
...
| Search v2 stable | Version 0.4.21 is public under npm tag `latest` and is active on Railway. The hosted and local package surfaces passed the maintained fixed suite and the incident-specific product matrix. The unchanged public MCP URL means existing hosted users need no configuration update. |
