...
| `P1` Shared deterministic understanding | Gate A release candidate prepared locally; migration smoke remains blocked; Gate B external approval has not started | Search intent core, generated rules/graph, shared policy-aware query-frame modules, recommendation clarification, maintained ranking policy, ambiguity diversification, all 50 approved SI brand classifications, rejected-alias enforcement, final-fusion policy enforcement, strict/prefer/all modes, isolated beta endpoint, additive audit contract, clarification-safe aggregation, prerelease routing, and rollback plan | Deterministic and 225-case quality checks passed; Deno checks passed; telemetry success and failure were exercised with a stub transport; package public-safety and allowlist checks passed; local candidate-search p95 was recorded; real PostgreSQL migration smoke was not run | Not packaged; the `@supericons/mcp` 0.4.18-beta.0 dry-run listed 38 files without publication | Not deployed | Not observed live | `references/verification/search-v2-deterministic-mcp-beta-gate-a-2026-07-12.md`; `references/verification/search-v2-deterministic-first-pivot-2026-07-12.md`; `references/verification/search-library-modes-2026-07-12.md`; `references/verification/search-query-frame-clarification-2026-07-12.md` |
...
For the deterministic-first scope, the next material step is a disposable-database migration smoke and RPC check. Gate B external approval follows only after that passes. Optional semantic phases `P3` through `P6` are paused and are not current release requirements.
...
1. Run `20260712_search_v2_beta_measurement.sql` in a disposable local or approved test database.
2. Verify migration apply, RPC success, invalid-label rejection, audit reads, and rollback ordering.
3. Re-run the Gate A contract, Deno, package, deterministic, and 225-case checks.
4. Prepare the Gate B approval request naming the endpoint, prerelease version, cohort, duration, adoption method, external mutations, and rollback target.
5. Do not deploy or publish until that request receives explicit owner approval.
