...
| Hosted access | The shared Supabase limiter currently uses an IP hash and a default 120 requests per minute. Tiered daily allowances (anonymous 300, registered free 1,500, paid 5,000 per client per day) are implemented in `rate-limit.ts` and wired into `handle-search-request.ts`, gated by `SEARCH_ENGINE_TIER_ENFORCEMENT` which defaults to off. This wiring is implemented and locally verified (deno check plus behavioral probe, 2026-07-19), not deployed. |
| Tier policy | `D-028`, `FR-43`, and `FR-44` define the intended public local core and tiered hosted service. The `CP-07` measurement artifact `docs/si-v2/search/experiments/hosted-allowance-measurement-2026-07-19.md` records the measured distribution and the chosen candidate thresholds. Public fair-use copy stating these limits was drafted in all 12 locales, then removed from the maintained docs sources on 2026-07-19 per independent review recommendation, pending owner ratification: the numbers publish only when enforcement and free-key issuance are live. The drafted copy is preserved in git history (commit `eb5d6878c`). Enforcement remains off pending the `FR-43` readiness conditions. |
