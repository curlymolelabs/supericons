...
| `P1` Shared deterministic understanding | Current main contains the reviewed Search v2 and Material integration. The first hosted search-only beta failed Gate C, was rolled back, and was not published. A replacement English local-first MCP prototype is implemented and locally verified on the preparation branch only. | Existing deterministic intelligence plus a beta-version and request-scoped local `search_icons` route; stable hosted fallback for localized, non-ASCII, and recommendation calls; packaged public synonym map and Material outline and solid SVGs; public local-runtime and index-date diagnostics; non-blocking tool-outcome telemetry | The clean 225-case fingerprint remains `ef293409...a76c8`. The local feasibility run measured 202.295 ms p95, 52,469,760 bytes combined RSS increase, and 62 zero results among 75 multilingual fixtures. Focused route checks prove eligible English and Material searches skip hosted search, fallbacks keep stable routing without a beta cohort, one outcome attempt is made for an eligible local call, and telemetry failure does not fail search. The earlier hosted Gate C failure remains recorded at 7,151.057 ms search p95 and one error among 38 captured requests. | A clean-install dry run contains 47 files, includes the public synonym map and 8,524-asset Material bundle, reproduces the fixed fingerprint, and resolves both Material styles from that bundle. The package is not published. | Migration `20260714180000` remains deployed from the failed hosted beta. The isolated beta endpoint was deleted. The local-first prototype needs no new function or migration. | The replacement prototype has not been observed live. Stable Railway MCP and Supabase function evidence is unchanged from the prior closeout. | `references/verification/search-v2-local-first-feasibility-2026-07-16.md`; `references/verification/search-v2-local-first-beta-prototype-2026-07-16.md`; `references/verification/search-v2-search-only-beta-gate-c-execution-2026-07-16.md` |
...
| Search v2 beta | Manifest `bf59e6cf...1d734` was executed. Hosted Gate C failed, the endpoint was deleted, and `0.4.19-beta.0` was not published. npm `latest` remains `0.4.17`. A replacement local-first prototype is locally verified but not independently approved or published. It requires no beta endpoint or new migration. The earlier Gate C harness corrections remain useful only if a later hosted measurement is approved. |
...
1. Independently review the local-first prototype, its decision and requirement change, and the revised measurement plan.
2. If the prototype passes review, prepare a new publication-only manifest and approval request. It must authorize no Supabase function or database change.
3. Keep localized, non-ASCII, and recommendation requests on stable hosted routes without a beta cohort. Keep web search unchanged.
4. Include a bounded informational local-versus-hosted result comparison in the beta scorecard, with the maximum hosted calls named in the approval request.
5. Count eligible local beta attempts from tool-outcome rows, and verify one outcome attempt per eligible tool call directly. Do not treat the hosted request dedupe key as coverage for this path.
