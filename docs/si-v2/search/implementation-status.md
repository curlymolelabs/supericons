...
| Search v2 beta | Manifest `bf59e6cf...1d734` was executed. Hosted Gate C failed, the endpoint was deleted, and `0.4.19-beta.0` was not published. npm `latest` remains `0.4.17`. The first local-first publication draft was stopped after a package-route parity gap was found. The correction and permanent stdio route gate are independently approved. Two later direct publication attempts stopped before creating the version, first on expected-absence handling and then because the npm account uses browser security-key 2FA rather than an authenticator code. One private staged upload then created the exact prerelease stage, but the runner stopped while parsing npm's success output before recording verification. Read-only list, view, download, hash, and 150-case smoke checks confirmed that exact private stage. The reconciliation packet permits zero further staged uploads and binds read-only stage recovery, one-use post-approval finalization, and a one-use hosted-comparison allowance. The prerelease is not public. The corrected beta requires no endpoint or migration. The earlier Gate C harness corrections remain useful only if a later hosted measurement is reviewed. |
...
1. Independently audit the zero-upload private-stage reconciliation runner and refreshed bound manifest.
