...
| `P1` Shared deterministic understanding | Search v2, Material support, and current main are reconciled and locally verified on an isolated integration branch; a fresh beta is not yet cut | Search intent and ranking policy; expressive related-result ordering; recommendation clarification; strict/prefer/all modes; legal English and localized workload limits; end-to-end tool latency logging; persisted worker evidence; SVG-free ordered candidate RPCs; shared recommendation candidate, metadata, semantic, SVG, and bulk-audit work; logical-query rate-limit accounting; public-safe in-band timing; Material ranking and fixed-preset SVG hydration; usage-event dedupe; guarded SQL runners; rollback plans | The combined Search v2 and Material gates pass through behavior commit `d8e80fb2b`. The clean 225-case fingerprint is `f5551b3b...380e2`; an exact comparison with the earlier reviewed integration found only the reviewed `swift` and `factory ai` catalog changes. Existing live Search v2 evidence still shows direct search at 1,647.287 ms p95 and localized search at 1,549.441 ms p95. The prior recommendation measurement remains invalid for the intended English workload because it used eight queries. | Stable package source `0.4.18` passes package inventory and clean-install gates; npm `latest` remains `0.4.17` in the saved read-only evidence | Saved live metadata reports `serve-material-snapshot` version 49, `mcp-search` version 38, and `search-icons` version 35 active. Saved evidence records Search v2 migrations through `20260714120000` and Material migrations `20260714220000` and `20260714223000`; the current Material migration ledger awaits a read-only database check. Migrations `20260714180000` and `20260714190000` remain local only. | Railway health was previously observed live at MCP version `0.4.18` with 8,524 Material assets. Search v2 beta behavior has not been observed live. | `references/verification/search-v2-main-reintegration-2026-07-16.md`; `references/verification/search-v2-material-integration-2026-07-16.md`; `references/verification/material-railway-recovery-narrow-completion-2026-07-16.md`; `references/verification/search-v2-roundtrip-latency-execution-2026-07-14.md` |
...
| --- | --- |
| Integration source | Search v2, Material, current main, and the expressive-fallback rule coexist through behavior commit `d8e80fb2b`. No release was made from the integration branch. |
...
1. Complete one read-only Supabase database check for the Material migration ledger before preparing a hosted release packet.
