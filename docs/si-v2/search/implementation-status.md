...
| `P1` Shared deterministic understanding | Approved deterministic behavior is locally complete; the beta measurement and rollback plan is prepared; isolated endpoint and audit prerequisites remain | Search intent core, generated rules/graph, shared policy-aware query-frame modules, recommendation clarification, maintained ranking policy, ambiguity diversification, all 50 approved SI brand classifications, rejected-alias enforcement, final-fusion policy enforcement, explicit strict/prefer/all search modes in local and hosted MCP code, and a controlled beta measurement and rollback plan | Deterministic quality checks passed; MCP default-package gate found zero external model-provider calls; npm dry-run listed 37 package files including the 7 required deterministic search files; beta plan was checked against current audit and package fields | Not packaged; the `@supericons/mcp` 0.4.17 dry-run contents were verified without creating or publishing a tarball | Not deployed | Not observed live | `references/verification/search-v2-deterministic-first-pivot-2026-07-12.md`; `references/verification/search-v2-brand-activation-and-evaluation-target-2026-07-12.md`; `references/verification/search-library-modes-2026-07-12.md`; `references/verification/search-query-frame-clarification-2026-07-12.md`; `references/verification/search-v2-deterministic-mcp-beta-plan-2026-07-12.md` |
...
For the deterministic-first scope, the next material step is local implementation of the isolated beta endpoint and missing audit fields defined in the beta plan. External deployment and publication remain subject to explicit approval. Optional semantic phases `P3` through `P6` are paused and are not current release requirements.
...
1. Implement a separate beta function endpoint without changing the production search endpoint.
2. Add and verify audit fields for library mode, search outcome, public confidence label, and beta cohort.
3. Prepare a prerelease MCP package under an explicit version or `beta` tag without changing `latest`.
4. Rerun the local quality, public-safety, package, failure-injection, and zero-provider-call gates.
5. Request explicit owner approval naming the endpoint, prerelease version, cohort, duration, and rollback target before any deployment or publication.
6. Run and close the beta using [`experiments/deterministic-mcp-beta-measurement-and-rollback-plan-2026-07-12.md`](experiments/deterministic-mcp-beta-measurement-and-rollback-plan-2026-07-12.md).
