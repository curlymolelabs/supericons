...
For the deterministic-first scope, the next material step is an explicitly approved controlled MCP deployment and measured live evidence. Optional semantic phases `P3` through `P6` are paused and are not current release requirements.
...
1. Verify the current deterministic policy set in a fresh local MCP package.
2. Prove the default package path makes no AI-agent, LLM, or embedding-provider call.
3. Prepare a controlled deterministic MCP beta and rollback record without publishing or deploying.
4. Request explicit owner approval before any deployment or publication.
5. Measure zero-result, low-confidence, exact-regression, latency, error, reformulation, and abuse signals with stable denominators.
