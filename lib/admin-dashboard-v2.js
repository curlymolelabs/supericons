...
export function compactDashboardV2EventRows(rows) {
  return rows.map((row) => ({
    ...
    result_count: row.result_count === null || row.result_count === undefined
      ? null
      : numeric(row.result_count),
    returned_icon_refs: boundedTextList(row.returned_icon_refs),
    returned_icon_refs_recorded: row.returned_icon_refs_recorded === true,
