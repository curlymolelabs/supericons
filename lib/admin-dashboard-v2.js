...
export function parseDashboardV2Range(url, now = new Date()) {
  ...
  if (custom) {
    ...
    const durationDays = Math.round((toMs - fromMs) / DAY_MS) + 1;
    ...
    from = days === null ? null : new Date(now.getTime() - days * DAY_MS).toISOString();
    toExclusive = null;
  }

  const fromMs = from ? Date.parse(from) : null;
  const toMs = toExclusive ? Date.parse(toExclusive) : now.getTime();
  const durationDays = fromMs === null ? null : Math.max(1, Math.ceil((toMs - fromMs) / DAY_MS));
  return {
    key,
    from,
    ...
    use_raw: durationDays !== null && durationDays < 90,
  };
}
...
export function buildDashboardV2Kpis(series, identityRows = []) {
  ...
  return {
    ...
    low_result_rate: lowEligible ? lowResultCount / lowEligible : 0,
...
export function normalizeDashboardV2QueryRows(rows) {
  return rows.map((row) => {
    ...
    return {
      ...
      result_count: row.minimum_result_count ?? (zero > 0 && success === 0 ? 0 : null),
...
export function compactDashboardV2QueryRows(rows) {
  return rows.map((row) => {
    const zero = row.true_zero_count > 0;
    const low = row.low_result_count > 0;
    return {
      query: row.query,
      library_filter: row.library_filter,
      ...
      client_label: first(row.estimated_client_keys, row.distinct_clients ? `${row.distinct_clients} clients` : 'Client count unavailable'),
      country_code: first(row.countries, 'Unknown'),
      channel: first(row.channels, 'unknown'),
      result_count: zero ? 0 : row.result_count,
      issue_type: zero ? 'zero_result' : low ? 'low_result' : 'successful',
