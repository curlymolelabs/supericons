...
export function parseDashboardV2Filters(url, now = new Date()) {
  const range = parseDashboardV2Range(url, now);
  ...
  return { ...range, channel, include_test: includeTest, q };
}
...
export function normalizeDashboardV2QueryRows(rows) {
  return rows.map((row) => {
    ...
    const hasResultCount = minimumResultCount !== null
    ...
    return {
      ...
      result_count: hasResultCount ? Number(minimumResultCount) : null,
      result_count_available: hasResultCount,
...
export function compactDashboardV2QueryRows(rows) {
  return rows.map((row) => {
    ...
    const clientMeasure = aggregateView ? 'client_days' : 'estimated_unique_clients';
    const resultCountAvailable = allZero || (!mixed && row.result_count_available === true);
    const resultCountKind = !allZero && !aggregateView && attempts > 1 && resultCountAvailable
      ? 'minimum_across_attempts'
      : resultCountAvailable
        ? 'exact'
        : 'unavailable';
    const countryCode = first(row.countries, null);
    ...
    return {
      query: row.query,
      library_filter: row.library_filter,
      query_origin: first(row.query_origins, 'legacy_unknown'),
      origin: first(row.query_origins, 'legacy_unknown'),
      visitor_kind: first(row.visitor_kinds, row.registered_user_count > 0 ? 'registered' : 'anonymous'),
      client_label: aggregateView
        ? row.distinct_clients
          ? `${row.distinct_clients} client-days`
          : 'Client-days unavailable'
        : first(row.estimated_client_keys, row.distinct_clients ? `${row.distinct_clients} clients` : 'Client count unavailable'),
      client_measure: clientMeasure,
      country_code: countryCode,
      country_available: Boolean(countryCode),
      country_reason: countryCode
        ? null
        : aggregateView
          ? 'Not available for aggregate view'
          : 'Country not recorded',
      channel: first(row.channels, 'unknown'),
      result_count: allZero ? 0 : resultCountAvailable ? row.result_count : null,
      result_count_available: resultCountAvailable,
      result_count_kind: resultCountKind,
      result_count_reason: resultCountAvailable
        ? resultCountKind === 'minimum_across_attempts'
          ? 'Minimum result count across grouped attempts'
          : null
        : aggregateView
          ? 'Not available for aggregate view'
          : mixed
