...
export function resolveGridHeadingText({
  ...
  const resolvedJobCategoryLabel = String(activeJobCategoryLabel || '');

  if (resolvedLibrary === 'all' && resolvedJobCategoryLabel) {
    return resolvedJobCategoryLabel;
  }

  if (resolvedJobCategoryLabel) {
    return `${resolvedLibraryTitle} + ${resolvedJobCategoryLabel}`;
  }
