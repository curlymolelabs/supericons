...
export function validateRegistryRecord(record) {
  ...
  for (const field of ['icon_id', 'source_library', 'source_name', 'label', 'purpose', 'use_when', 'version', 'status', 'access_tier', 'projection_policy']) {
    assertNonEmptyString(record, field);
  }

  if ('avoid_when' in record) {
    assertNonEmptyString(record, 'avoid_when');
  }
  ...
  if ('asset_type' in record) {
    assertNonEmptyString(record, 'asset_type');
  }

  if ('motion' in record) {
    if (!record.motion || typeof record.motion !== 'object' || Array.isArray(record.motion)) {
      throw new Error('Invalid motion');
    }
    if (typeof record.motion.has_motion !== 'boolean') {
      throw new Error('Invalid motion.has_motion');
    }
  }
