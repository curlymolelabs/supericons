...
export function buildRegistrySummary(records, schemaVersion) {
...
export function buildRegistryProjections(sourceRecords, { schemaVersion }) {
  const normalizedRecords = sortRecords(sourceRecords.map((record) => normalizeRegistryRecord(record)));
  const summary = buildRegistrySummary(normalizedRecords, schemaVersion);
