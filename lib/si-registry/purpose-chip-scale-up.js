...
function clampRoutingScore(value) {
...
function estimateScaleUpRoutingScore(baseRoutingScore, profileStrength, visualPayloadStatus) {
  const base = typeof baseRoutingScore === 'number' ? baseRoutingScore : 0.72;
  ...
  return clampRoutingScore(base + strengthBoost + visualBoost);
}

function determineNextStep(routingScore, profileStrength, visualPayloadStatus) {
  const visualReady = isVisualPayloadReady(visualPayloadStatus);

  if (visualReady && profileStrength === 'high' && routingScore >= 0.86) {
    return 'editor_review';
  }

  if (visualReady && routingScore >= 0.72) {
    return 'visual_review';
  }

  if (!visualReady && routingScore >= 0.74 && profileStrength !== 'low') {
    return 'text_review';
  }
...
export function buildAutomationStagedRecord(candidateRecord, visualReviewInput) {
  ...
  const routingScore = estimateScaleUpRoutingScore(candidateRecord.routing_score, semanticProfile.strength, visualReviewInput.visual_payload_status);
...
export function buildAutomationNextStepEntry(stagedRecord, visualReviewInput) {
  const nextStep = determineNextStep(
    stagedRecord.routing_score ?? 0,
    resolveSemanticProfile(stagedRecord).strength || 'low',
    visualReviewInput.visual_payload_status
  );
  ...
  return {
    ...
    routing_score: stagedRecord.routing_score ?? 0,
    next_step: nextStep,
...
export function buildPurposeChipScaleUpArtifacts({ worklist = [], candidateRecords = [], visualReviewInputs = [], approvedRecords = [], editorHoldQueue = [], promotionDecisions = {} }) {
  ...
  nextSteps.sort((left, right) => {
    ...
    const routingScoreDelta = (right.routing_score ?? 0) - (left.routing_score ?? 0);
