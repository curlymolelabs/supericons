...
// Tier projection uses the C1 literal section names. The existing pilot
// records remain readable through DESIGN_PATH_ALIASES until their two-phase
// migration moves restricted fragments out of git.
//
...
export const PUBLIC_DESIGN_PATHS = Object.freeze([
  ...
  'visual.depicts', 'visual.variants[].id', 'visual.variants[].depicts',
  'visual.style_renders',
  'semantics.purpose', 'semantics.semantic_tags', 'semantics.synonyms',
  'behavior.motion.has_motion', 'behavior.motion.behavior',
  'actions.actions', 'actions.mcp',
  'economics.access_tier', 'economics.gated_price',
  'economics.payment_rails', 'economics.license',
]);

export const GATED_DESIGN_PATHS = Object.freeze([
  'visual.production_assets', 'visual.render_overrides',
  'semantics.must_communicate', 'semantics.mindmap.associations',
  'semantics.mindmap.metaphor_candidates',
  'semantics.mindmap.anti_associations', 'semantics.distinct_from',
  'geometry', 'behavior.motion.spec', 'actions.service_endpoints',
  'actions.integration_recipes', 'economics.license_gated',
  'process.revision_history',
]);

export const INTERNAL_DESIGN_PATHS = Object.freeze([
  'economics.revenue_splits', 'community.external_comments',
  'process.evidence', 'process.editorial_notes',
]);

// Longest aliases come first so the public synonyms path leaves the otherwise
// gated mind map before the root-level soul alias applies.
export const DESIGN_PATH_ALIASES = Object.freeze([
  Object.freeze(['soul.mindmap.synonyms', 'semantics.synonyms']),
  Object.freeze(['external_comments', 'community.external_comments']),
  Object.freeze(['revision_history', 'process.revision_history']),
  Object.freeze(['editorial_notes', 'process.editorial_notes']),
  Object.freeze(['construction', 'geometry']),
  Object.freeze(['evidence', 'process.evidence']),
  Object.freeze(['face', 'visual']),
  Object.freeze(['soul', 'semantics']),
  Object.freeze(['pulse', 'behavior']),
  Object.freeze(['hands', 'actions']),
  Object.freeze(['wallet', 'economics']),
]);

// Pack ownership follows the v2 pack rule: identity and advertised render
// names are public; the reusable design system is gated; detailed source
// provenance remains internal.
export const PUBLIC_PACK_PATHS = Object.freeze([
  'pack_id', 'name', 'tagline', 'version', 'status', 'member_ids',
  'design_language.render_styles.public_names',
  'design_language.render_styles.recipes_tier',
  'provenance.created', 'provenance.review_rounds_total',
]);

export const GATED_PACK_PATHS = Object.freeze([
  'design_language.grammar', 'design_language.territory_map',
  'design_language.style_tokens', 'design_language.motion_language',
  'design_language.craft_rules',
]);

export const INTERNAL_PACK_PATHS = Object.freeze([
  'provenance.source',
]);
...
export function normalizeDesignPath(path) {
  for (const [legacy, literal] of DESIGN_PATH_ALIASES) {
    if (pathStartsWith(path, legacy)) return `${literal}${path.slice(legacy.length)}`;
  }
  return path;
}
...
function collectLeafPaths(value, path = '', paths = []) {
  if (Array.isArray(value)) {
    ...
    for (const item of value) collectLeafPaths(item, `${path}[]`, paths);
    return paths;
  }
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [key, item] of entries) {
      collectLeafPaths(item, path ? `${path}.${key}` : key, paths);
    }
    return paths;
  }
...
function tierMapFor(kind) {
  if (kind === 'icon') {
    return {
      public: PUBLIC_DESIGN_PATHS,
      gated: GATED_DESIGN_PATHS,
      internal: INTERNAL_DESIGN_PATHS,
    };
  }
  if (kind === 'pack') {
    return {
      public: PUBLIC_PACK_PATHS,
      gated: GATED_PACK_PATHS,
      internal: INTERNAL_PACK_PATHS,
    };
  }
  fail(`Unknown tier-map kind: ${kind}`);
}

export function classifyRecordTierPaths(record, kind) {
  assertObject(record, `${kind} record`);
  const tierMap = tierMapFor(kind);
  const assignments = [];
  const seen = new Set();

  for (const sourcePath of collectLeafPaths(record)) {
    if (seen.has(sourcePath)) continue;
    seen.add(sourcePath);
    const literalPath = kind === 'icon' ? normalizeDesignPath(sourcePath) : sourcePath;
    const tiers = Object.entries(tierMap)
      .filter(([, allowedPaths]) => allowedPaths.some((allowed) => pathStartsWith(literalPath, allowed)))
      .map(([tier]) => tier);

    if (tiers.length === 0) {
      fail(`Unmapped ${kind} field: ${sourcePath} (C1 path: ${literalPath})`);
    }
    if (tiers.length > 1) {
      fail(`Multi-tier ${kind} field: ${sourcePath} maps to ${tiers.join(', ')}`);
    }
    assignments.push(Object.freeze({ sourcePath, literalPath, tier: tiers[0] }));
  }

  return Object.freeze(assignments);
}

export function summarizeTierCoverage(record, kind) {
  const assignments = classifyRecordTierPaths(record, kind);
  const summary = { public: 0, gated: 0, internal: 0, total: assignments.length };
  for (const assignment of assignments) summary[assignment.tier] += 1;
