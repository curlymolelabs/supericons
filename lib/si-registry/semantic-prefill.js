...
function estimateRoutingScore(worklistItem, label, sourceName, laneConfig) {
  ...
  const base = laneConfig.routing_seed;
  const routingScore = base + strongBoost + softBoost - rarityPenalty;

  return Number(Math.max(0.45, Math.min(0.94, routingScore)).toFixed(2));
}
...
export function buildPurposeChipCandidateRecord(worklistItem, iconRecord) {
  ...
  return {
    ...
    routing_score: estimateRoutingScore(worklistItem, label, sourceName, laneConfig),
  };
}
...
export function buildPurposeChipVisualReviewInputs(candidateRecords, iconIndexMap) {
  return candidateRecords.map((candidateRecord) => {
    ...
    return {
      ...
      candidate_routing_score: candidateRecord.routing_score,
