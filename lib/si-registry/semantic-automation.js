...
function buildDraftFromReference(iconRecord, sourceName, referenceRecord) {
  ...
  const candidateRecord = {
    icon_id: `mingcute:${normalizeSourceName(sourceName)}`,
    source_group: 'free',
...
function profileFromTokens(tokens) {
  ...
  return {
    label: buildLabelFromTokens(tokens) || 'Control',
    purpose: 'Show a general interface control or state cue.',
    category: 'navigation_interface',
    intent: 'inform',
    domain: 'ui_controls',
    semantic_tags: uniqueStrings(tokens),
    synonyms: uniqueStrings(tokens.map((token) => token.replace(/_/g, ' '))),
    use_when: 'Use when the icon acts as a straightforward interface control or visible state cue.',
    avoid_when: 'Do not use when the product needs a more specific approved meaning than this generic draft can support.',
    depicts: 'A general interface symbol used for a straightforward control or state cue.',
    confidence: 0.7,
  };
}

function buildHeuristicDraft(iconRecord, sourceName) {
  const tokens = tokenize(sourceName);
...
function buildWorklistItem(iconRecord, selectionScore, familyKey, approvedReference, rank) {
  return {
    rank,
    icon_id: `mingcute:${normalizeSourceName(iconRecord.id)}`,
    source_library: 'mingcute',
    source_asset_name: iconRecord.id,
    source_name: normalizeSourceName(iconRecord.id),
...
export function selectSemanticAutomationBatchIcons(iconIndexEntries, approvedRecords, batchConfig, options = {}) {
  ...
  return selected.map((candidate, index) => {
    ...
    const candidateRecord = candidate.approvedReference
      ? buildDraftFromReference(candidate.entry, candidate.entry.id, candidate.approvedReference)
      : buildHeuristicDraft(candidate.entry, candidate.entry.id);
