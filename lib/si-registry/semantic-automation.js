...
const BRAND_PLATFORM_DOMAIN = 'brand_platforms';

const SIMPLE_ICON_DOMAIN_HINTS = Object.freeze({
...
function detectSimpleIconsDomain(slug, titleTokens) {
  const haystack = new Set([slug, ...titleTokens]);

  for (const [domain, hints] of Object.entries(SIMPLE_ICON_DOMAIN_HINTS)) {
    if (hints.some((hint) => haystack.has(normalizeSourceName(hint)))) {
      return domain;
    }
  }

  return BRAND_PLATFORM_DOMAIN;
}

function buildSimpleIconsConfidence(metadata, slug, titleTokens) {
  let confidence = 0.84;

  if (metadata?.source) confidence += 0.03;
  if (metadata?.guidelines) confidence += 0.03;
  if (metadata?.title) confidence += 0.02;
  if (SIMPLE_ICON_AMBIGUOUS_SLUGS.has(slug)) confidence -= 0.1;
  if (titleTokens.length <= 1 && titleTokens[0] && titleTokens[0].length <= 2) confidence -= 0.06;

  return Math.max(0.66, Math.min(0.92, confidence));
}

function buildSimpleIconsSelectionScore(metadata, domain, confidence, slug) {
  let score = Math.round(confidence * 10);
  if (metadata?.source) score += 2;
  if (metadata?.guidelines) score += 3;
  if (domain !== BRAND_PLATFORM_DOMAIN) score += 2;
...
function buildSimpleIconsDraft(iconRecord) {
  ...
  const domain = detectSimpleIconsDomain(sourceName, titleTokens);
  const confidence = buildSimpleIconsConfidence(metadata, sourceName, titleTokens);
  ...
  return {
    ...
    candidateRecord,
    familyKey: domain,
    selectionScore: buildSimpleIconsSelectionScore(metadata, domain, confidence, sourceName),
  };
}
...
function buildDraftFromReference(iconRecord, sourceName, referenceRecord, libraryId = 'mingcute') {
  ...
  const candidateRecord = {
    ...
    intent: referenceRecord.intent,
    domain: referenceRecord.domain,
...
function profileFromTokens(tokens) {
  ...
  if (hasToken(tokens, 'search')) {
    ...
    return {
      ...
      intent: 'discover',
      domain: hasToken(tokens, 'ai') ? 'ai_agents' : hasToken(tokens, 'code') ? 'developer_tools' : 'ui_controls',
  ...
  if (hasAnyToken(tokens, ['menu', 'sidebar', 'dashboard', 'grid', 'list', 'home', 'fullscreen', 'more'])) {
    ...
    return {
      ...
      purpose,
      category: 'navigation_interface',
      intent: hasToken(tokens, 'home') || hasToken(tokens, 'dashboard') ? 'navigate' : 'control',
      domain: 'ui_shell',
  ...
  if (hasAnyToken(tokens, ['check', 'alert', 'warning', 'info', 'ban', 'eye', 'bell'])) {
    ...
    return {
      ...
      intent: hasAnyToken(tokens, ['alert', 'warning']) ? 'warn' : 'inform',
  ...
  if (hasAnyToken(tokens, ['message', 'mail', 'send'])) {
    return {
      ...
      intent: hasToken(tokens, 'send') ? 'act' : 'inform',
      domain: hasToken(tokens, 'mail') ? 'communication' : 'communication',
  ...
  if (hasAnyToken(tokens, ['file', 'folder', 'download', 'upload', 'open', 'link'])) {
    ...
    return {
      ...
      intent: hasAnyToken(tokens, ['download', 'upload', 'open']) ? 'act' : 'inform',
      domain: 'ui_controls',
...
function selectUiSemanticBatchIcons(iconIndexEntries, approvedRecords, batchConfig, options = {}) {
  ...
  const candidates = (iconIndexEntries || [])
    ...
    .filter((item) => !excludedIconIds.has(`mingcute:${item.sourceName}`))
  ...
  return selected.map((candidate, index) => {
    ...
    if (!candidate.approvedReference && candidateRecord.confidence > 0.86) {
      candidateRecord.confidence = 0.86;
    }
...
export function buildSemanticAutomationBatchArtifacts({ iconIndexEntries, approvedRecords, batchConfig, excludedIconIds = [] }) {
  ...
  const summary = {
    ...
    confidence_band_counts: reviewSummary.byBand,
  };
