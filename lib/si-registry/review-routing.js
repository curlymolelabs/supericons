...
export const DEFAULT_THRESHOLD_HIGH_CONFIDENCE = 0.82;
...
export function getConfidenceBand(confidence, thresholds = {}) {
  const normalizedConfidence = toConfidenceNumber(confidence) ?? 0;
  const highConfidenceThreshold = thresholds.highConfidenceThreshold ?? DEFAULT_THRESHOLD_HIGH_CONFIDENCE;
  ...
  if (normalizedConfidence >= highConfidenceThreshold) {
    return CONFIDENCE_BANDS.HIGH_CONFIDENCE;
  }

  if (normalizedConfidence >= needsReviewThreshold) {
    return CONFIDENCE_BANDS.NEEDS_REVIEW;
  }

  return CONFIDENCE_BANDS.AMBIGUOUS;
}
...
export function routeCandidateForReview(candidate, options = {}) {
  ...
  const confidenceBand = getConfidenceBand(confidence, options);
  ...
  } else if (conflictSignals.length > 0 || confidenceBand === CONFIDENCE_BANDS.AMBIGUOUS) {
    queueOutcome = REVIEW_QUEUE_OUTCOMES.ESCALATE_TO_STRONGER_REVIEW;
  } else if (confidenceBand === CONFIDENCE_BANDS.NEEDS_REVIEW) {
...
export function summarizeReviewQueue(items) {
  ...
  const initialByBand = Object.values(CONFIDENCE_BANDS).reduce((summary, band) => {
    summary[band] = 0;
    return summary;
  }, {});
  ...
  return (items || []).reduce(
    (summary, item) => {
      ...
      summary.byBand[item.confidence_band] = (summary.byBand[item.confidence_band] || 0) + 1;
      return summary;
    },
    {
      count: 0,
