...
export function buildFinalRecordsFromDepictsOnlyAgainstBaseline({
  baselineRecords,
  agentDepicts,
  expectedIconIds = null,
}) {
  const baselineById = new Map(baselineRecords.map((record) => [record.icon_id, record]));
  ...
  const records = agentDepicts.map((item) => {
    ...
    const baseline = baselineById.get(item.icon_id);
    if (!baseline) {
...
export function buildFinalRecordsFromDepictsOnly({ liveRecords, agentDepicts, expectedIconIds = null }) {
  return buildFinalRecordsFromDepictsOnlyAgainstBaseline({
    baselineRecords: liveRecords,
    agentDepicts,
    expectedIconIds,
  });
}
