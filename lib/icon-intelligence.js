...
async function maybeLogReplacement({
  ...
  await postRpc('si_log_icon_evidence', {
...
export async function logNoResultsFeedback({
  searchQuery = null,
  feedbackText = null,
  libraryFilter = null,
  jobCategory = null,
  ...
  if (!normalizedQuery || normalizedQuery.length < 2 || !safeFeedbackText) {
    return false;
  }
  ...
  try {
    ...
    await postRpc('si_log_icon_evidence', {
      ...
      p_evidence_text: safeFeedbackText,
      p_session_hash: sessionHash,
      p_created_at: new Date().toISOString(),
    });
    return true;
