...
export function getSearchInterpretationPlan(query) {
  ...
  const families = familyIds.map((familyId) => familyById.get(familyId)).filter(Boolean);
...
export function getMeaningPolicyPenalty(query, candidate = {}) {
  ...
  return (match.queryPolicy.avoid_candidate_terms || []).some((term) => (
...
export function getCandidateInterpretationFamilyIds(query, candidate = {}) {
  ...
  for (const family of plan.families) {
    ...
    if (matchedVariant || matchedCandidate || matchedBrand) familyIds.push(family.id);
  }
