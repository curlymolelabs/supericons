...
import { buildHostedAnimatedSvgResponse, parseHostedAnimatedSvgRequest } from '../_shared/motion-lab/runtime.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const request = parseHostedAnimatedSvgRequest(body);
    return buildJsonResponse(buildHostedAnimatedSvgResponse(request));
  } catch (error) {
