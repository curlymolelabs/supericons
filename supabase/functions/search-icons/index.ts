...
import { buildPrivateRowMaps } from '../_shared/search-engine/catalog.ts';
import { normalizeQuery } from '../_shared/search-engine/normalize.ts';
import { rerankCandidates } from '../_shared/search-engine/rank.ts';
import { enforceSearchRateLimit, SearchEngineHttpError } from '../_shared/search-engine/rate-limit.ts';
import type { CandidateRow, PrivateFeatureRow, PrivateManifestRow } from '../_shared/search-engine/types.ts';

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-si-session-hash',
};
