...
import { buildHostedRecipeResponse, parseHostedRecipeRequest } from '../_shared/motion-lab/runtime.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const request = parseHostedRecipeRequest(body);
    return buildJsonResponse(buildHostedRecipeResponse(request));
  } catch (error) {
