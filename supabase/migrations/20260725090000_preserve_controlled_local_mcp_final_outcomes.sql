-- Preserve controlled Local MCP traffic when usage events become final outcomes.
--
-- Rollback plan, written before implementation:
-- 1. Run the matching rollback file to restore the prior trigger classification logic.
-- 2. Keep corrected historical final rows as controlled_test because reverting them
--    would knowingly reintroduce false production traffic.
-- 3. Keep all source usage and final-outcome records.
-- 4. Set search_telemetry_settings.dashboard_source to legacy only if the broader
--    admin release also needs to be rolled back.
--
-- Backward compatibility:
-- - No table, column, index, public function signature, or search response changes.
-- - Existing writers may continue sending the same payloads.
-- - Missing or ordinary cohorts keep their current traffic classification.
-- - The data correction uses the exact source_event_id link, never query or time.

begin;

update public.search_final_outcomes as final
set traffic_class = 'controlled_test'
from public.mcp_usage_events as usage
where final.channel = 'local_mcp'
  and final.source_event_id = 'mcp_usage_events:' || usage.id::text
  and (
    lower(trim(coalesce(usage.beta_cohort, ''))) like 'controlled-run:%'
    or position(
      ':founder_controlled' in lower(trim(coalesce(usage.beta_cohort, '')))
    ) > 0
    or position(
      ':controlled_' in lower(trim(coalesce(usage.beta_cohort, '')))
    ) > 0
  )
  and final.traffic_class <> 'controlled_test';
...
begin
  ...
  v_beta_cohort := lower(trim(coalesce(new.beta_cohort, '')));
  if v_beta_cohort like 'controlled-run:%'
     or position(':founder_controlled' in v_beta_cohort) > 0
     or position(':controlled_' in v_beta_cohort) > 0 then
    v_traffic_class := 'controlled_test';
  else
...
comment on function public.si_capture_mcp_search_final_outcome() is
  'Copies eligible MCP outcomes into the final ledger and preserves trusted traffic classification.';
