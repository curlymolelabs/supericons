-- Record stable Local MCP search outcomes in the existing usage ledger.
--
-- Rollback plan:
-- 1. Restore the stable search_icons suppression with the matching rollback file.
-- 2. Keep all records already written to mcp_usage_events and search_final_outcomes.
-- 3. Do not change the published npm package because it already calls this function.
...
comment on function public.si_log_mcp_search_outcome_v2(
  ...
  integer,
  timestamptz
) is 'Public-safe Local MCP outcome logger for stable and prerelease package traffic.';
