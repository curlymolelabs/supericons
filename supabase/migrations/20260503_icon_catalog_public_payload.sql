alter table public.icon_catalog
  add column if not exists svg text;

drop function if exists public.si_search_icon_candidates(text, text, integer);

create function public.si_search_icon_candidates(
  p_query text,
