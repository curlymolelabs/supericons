...
function getGeneratedPhraseIntentMatches(normalizedQuery) {
  if (!normalizedQuery) return [];

  const paddedQuery = ` ${normalizedQuery} `;
  return Object.entries(INTENT_RULES)
    .filter(([term]) => term.includes(' ') && paddedQuery.includes(` ${term} `))
    .sort(([left], [right]) => right.length - left.length)
    .map(([term, rule]) => ({ token: term, rule, compound: true }));
}
...
export function buildSearchIntentProfile(query) {
  ...
  const activeRules = [
    ...getCompoundIntentMatches(normalized),
    ...getGeneratedPhraseIntentMatches(normalized),
  ];
