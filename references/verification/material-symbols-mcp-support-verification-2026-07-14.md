...
| Production latency baseline | Packet 3R guarded runner and retained artifacts | Invalidated after execution | The commands and original gates passed, but later semantic review proved the evidence unsuitable for comparison. The grouped recommendation path swallowed an unsupported transport response and returned no recommendation in all 21 samples. The search artifact encoded no semantic outcomes, included 10 zero-result warm samples, and already exceeded the proposed absolute gate. Retain both artifacts as defect evidence only. |
...
2. Run a separately approved baseline through the real production transport with semantic outcome gates. Define the replacement latency limits from that evidence. Do not reuse the invalidated 3,337.062 ms or 459.204 ms comparisons.
