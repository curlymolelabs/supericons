...
async function getLucideBatchIds() {
  const entries = await fs.readdir(automationRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && /^lucide-batch-\d+-selection\.json$/i.test(entry.name))
    .map((entry) => entry.name.replace(/-selection\.json$/i, ''))
    .sort((left, right) => left.localeCompare(right, undefined, { numeric: true }));
}

function buildEditorBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^lucide-batch-(\d+)$/i);
  if (!match) {
    throw new Error(`Cannot derive Lucide editor batch id from ${sourceBatchId}`);
  }
  return `lucide-editor-review-batch-${match[1].padStart(2, '0')}`;
}

function buildVisualBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^lucide-batch-(\d+)$/i);
  if (!match) {
    throw new Error(`Cannot derive Lucide visual batch id from ${sourceBatchId}`);
  }
  return `lucide-visual-review-batch-${match[1].padStart(2, '0')}`;
}

async function getResolvedIds() {
  const decisions = await readJsonOrDefault(path.join(automationRoot, 'lucide', 'promotion-decisions.json'), {
...
async function getStagedLucideCount(batchIds) {
  let count = 0;
...
async function processExistingBatches(batchIds) {
  ...
  for (const batchId of batchIds) {
    ...
    if (readyIds.length > 0) {
      runNodeScript('build-lucide-editor-review-batch.mjs', buildEditorBatchId(batchId), batchId);
      didWork = true;
    }
    if (visualIds.length > 0) {
      runNodeScript('build-lucide-visual-review-batch.mjs', buildVisualBatchId(batchId), batchId);
      didWork = true;
    }
  }

  if (didWork) {
    runNodeScript('build-lucide-approved-records.mjs');
  }
...
async function stageNextBatch() {
  const beforeBatchIds = await getLucideBatchIds();
  const beforeSet = new Set(beforeBatchIds);
  runNodeScript('build-next-lucide-batch-selection.mjs');
  const afterBatchIds = await getLucideBatchIds();
...
async function main() {
  ...
  while (iterations < 200) {
    iterations += 1;
    const batchIds = await getLucideBatchIds();
    const processed = await processExistingBatches(batchIds);
    if (processed) {
      continue;
    }

    const stagedCount = await getStagedLucideCount(batchIds);
    if (stagedCount >= lucideTotal) {
      break;
    }
  ...
  runNodeScript('verify-lucide-approved-records.mjs');
  ...
  console.log(
    `run-lucide-library-rollout: approved=${approvalSummary.total_approved_records} | hold=${approvalSummary.total_editor_hold_records} | draft=${approvalSummary.total_reviewed_drafts} | public_free=${registrySummary.publicRecordCount}`
  );
}
