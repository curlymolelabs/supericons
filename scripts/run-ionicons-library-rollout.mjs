...
async function getIoniconsBatchIds() {
  const entries = await fs.readdir(automationRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && /^ionicons-batch-\d+-selection\.json$/i.test(entry.name))
    .map((entry) => entry.name.replace(/-selection\.json$/i, ''))
    .sort((left, right) => left.localeCompare(right, undefined, { numeric: true }));
}

function buildEditorBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^ionicons-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Ionicons editor batch id from ${sourceBatchId}`);
  return `ionicons-editor-review-batch-${match[1].padStart(2, '0')}`;
}

function buildVisualBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^ionicons-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Ionicons visual batch id from ${sourceBatchId}`);
  return `ionicons-visual-review-batch-${match[1].padStart(2, '0')}`;
}

async function getResolvedIds() {
  const decisions = await readJsonOrDefault(path.join(automationRoot, 'ionicons', 'promotion-decisions.json'), {
...
async function getStagedIoniconsCount(batchIds) {
  let count = 0;
...
async function processExistingBatches(batchIds) {
  ...
  for (const batchId of batchIds) {
    ...
    if (readyIds.length > 0) {
      runNodeScript('build-ionicons-editor-review-batch.mjs', buildEditorBatchId(batchId), batchId);
      didWork = true;
    }
    if (visualIds.length > 0) {
      runNodeScript('build-ionicons-visual-review-batch.mjs', buildVisualBatchId(batchId), batchId);
      didWork = true;
    }
  }

  if (didWork) {
    runNodeScript('build-ionicons-approved-records.mjs');
  }
...
async function stageNextBatch() {
  const beforeBatchIds = await getIoniconsBatchIds();
  const beforeSet = new Set(beforeBatchIds);
  runNodeScript('build-next-ionicons-batch-selection.mjs');
  const afterBatchIds = await getIoniconsBatchIds();
...
async function main() {
  const ioniconsTotal = (publicIconIndex.icons || []).filter((icon) => icon.lib === 'ionicons').length;
  let iterations = 0;

  while (iterations < 400) {
    iterations += 1;
    const batchIds = await getIoniconsBatchIds();
    const processed = await processExistingBatches(batchIds);
    if (processed) {
      continue;
    }

    const stagedCount = await getStagedIoniconsCount(batchIds);
    if (stagedCount >= ioniconsTotal) {
      break;
    }
  ...
  runNodeScript('verify-ionicons-approved-records.mjs');
  ...
  console.log(
    `run-ionicons-library-rollout: approved=${approvalSummary.total_approved_records} | hold=${approvalSummary.total_editor_hold_records} | draft=${approvalSummary.total_reviewed_drafts} | public_free=${registrySummary.publicRecordCount}`
  );
}
