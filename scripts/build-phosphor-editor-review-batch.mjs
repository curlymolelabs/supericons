...
const BATCH_ID = process.argv[2] || 'phosphor-editor-review-batch-01';
...
const reviewedRecords = selectedIds.map((iconId) => {
  ...
  if (!candidateRecord) {
      throw new Error(`Missing Phosphor candidate record for ${iconId}`);
  }
...
const batchData = {
  ...
  purpose: 'Approve the high-confidence Phosphor editor-review queue from the current Phosphor automation batch.',
...
This batch clears the high-confidence Phosphor editor-review queue from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-phosphor-editor-review-batch: reviewed=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
