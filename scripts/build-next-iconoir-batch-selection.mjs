...
const TEMPLATE_PATH = path.join(automationRoot, 'iconoir-template.json');
...
function getIconoirBatchIds(batchIds) {
  return batchIds.filter((batchId) => /^iconoir-batch-\d+$/i.test(batchId));
}
...
function getNextBatchId(batchIds) {
  const maxNumber = batchIds.reduce((currentMax, batchId) => {
    const match = batchId.match(/^iconoir-batch-(\d+)$/i);
    if (!match) return currentMax;
    return Math.max(currentMax, Number(match[1]));
  }, 0);
  return `iconoir-batch-${String(maxNumber + 1).padStart(2, '0')}`;
}
...
function buildPhaseConfigs(template, nextBatchId, targetConfig, excludeBatchIds) {
  ...
  return [
    {
      phaseId: 'high_signal',
      description: 'Continue the Iconoir rollout with the strongest remaining UI and product-control icons.',
      config: {
        ...common,
        score_threshold: 2,
    ...
    {
      phaseId: 'relaxed',
      description: 'Broaden Iconoir coverage while still avoiding the obviously decorative long-tail shapes.',
      config: {
        ...common,
        score_threshold: 0,
        exclude_tokens: [...(template.exclude_tokens || [])],
    ...
    {
      phaseId: 'full_sweep',
      description: 'Sweep the remaining Iconoir library so the rollout can fully close.',
      config: {
        ...common,
        score_threshold: -10,
        exclude_tokens: [],
...
async function main() {
  const existingBatchIds = getIconoirBatchIds(await listSemanticAutomationBatchIds());
  ...
  const remainingCount = iconoirTotal - priorWorklistIds.size;

  if (remainingCount <= 0) {
    console.log('build-next-iconoir-batch-selection: Iconoir is already fully staged.');
    return;
  }
  ...
  if (!selectedPhase || !selectedArtifacts) {
    throw new Error(`Could not build a valid Iconoir selection for ${nextBatchId}`);
  }
  ...
  console.log(
    `build-next-iconoir-batch-selection: batch=${nextBatchId} | phase=${selectedPhase.phaseId} | selected=${selectedArtifacts.summary.selected_count} | remaining=${remainingCount}`
  );
}
