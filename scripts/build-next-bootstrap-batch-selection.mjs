...
const TEMPLATE_PATH = path.join(automationRoot, 'bootstrap-template.json');
...
function getBootstrapBatchIds(batchIds) {
  return batchIds.filter((batchId) => /^bootstrap-batch-\d+$/i.test(batchId));
}
...
function getNextBatchId(batchIds) {
  const maxNumber = batchIds.reduce((currentMax, batchId) => {
    const match = batchId.match(/^bootstrap-batch-(\d+)$/i);
    if (!match) return currentMax;
    return Math.max(currentMax, Number(match[1]));
  }, 0);
  return `bootstrap-batch-${String(maxNumber + 1).padStart(2, '0')}`;
}
...
function buildPhaseConfigs(template, nextBatchId, targetConfig, excludeBatchIds) {
  const common = {
    ...
    library_id: 'bootstrap',
    library_label: 'Bootstrap',
  ...
  return [
    {
      phaseId: 'high_signal',
      description: 'Continue the Bootstrap rollout with the strongest remaining UI and product-control icons.',
      config: {
        ...common,
        score_threshold: 2,
    ...
    {
      phaseId: 'relaxed',
      description: 'Broaden Bootstrap coverage while still avoiding the obviously decorative long-tail shapes.',
      config: {
        ...common,
        score_threshold: 0,
        exclude_tokens: [...(template.exclude_tokens || [])],
    ...
    {
      phaseId: 'full_sweep',
      description: 'Sweep the remaining Bootstrap library so the rollout can fully close.',
      config: {
        ...common,
        score_threshold: -10,
        exclude_tokens: [],
...
async function main() {
  const existingBatchIds = getBootstrapBatchIds(await listSemanticAutomationBatchIds());
  ...
  const remainingCount = bootstrapTotal - priorWorklistIds.size;

  if (remainingCount <= 0) {
    console.log('build-next-bootstrap-batch-selection: Bootstrap is already fully staged.');
    return;
  }
  ...
  if (!selectedPhase || !selectedArtifacts) {
    throw new Error(`Could not build a valid Bootstrap selection for ${nextBatchId}`);
  }
  ...
  console.log(
    `build-next-bootstrap-batch-selection: batch=${nextBatchId} | phase=${selectedPhase.phaseId} | selected=${selectedArtifacts.summary.selected_count} | remaining=${remainingCount}`
  );
}
