...
const TEMPLATE_PATH = path.join(automationRoot, 'tabler-batch-01-selection.json');
const DEFAULT_TARGET = { target_size: 240, target_min: 200, target_max: 320 };
...
function getHeroiconsBatchIds(batchIds) {
  return batchIds.filter((batchId) => /^heroicons-batch-\d+$/i.test(batchId));
}
...
function getNextBatchId(batchIds) {
  const maxNumber = batchIds.reduce((currentMax, batchId) => {
    const match = batchId.match(/^heroicons-batch-(\d+)$/i);
    if (!match) return currentMax;
    return Math.max(currentMax, Number(match[1]));
  }, 0);
  return `heroicons-batch-${String(maxNumber + 1).padStart(2, '0')}`;
}
...
function buildPhaseConfigs(template, nextBatchId, targetConfig, excludeBatchIds) {
  ...
  return [
    {
      phaseId: 'high_signal',
      description: 'Continue the Heroicons rollout with the strongest remaining UI and product-control icons.',
      config: {
        ...common,
        score_threshold: 2,
    ...
    {
      phaseId: 'relaxed',
      description: 'Broaden Heroicons coverage while still avoiding the obviously decorative long-tail shapes.',
      config: {
        ...common,
        score_threshold: 0,
        exclude_tokens: [...(template.exclude_tokens || [])],
    ...
    {
      phaseId: 'full_sweep',
      description: 'Sweep the remaining Heroicons library so the rollout can fully close.',
      config: {
        ...common,
        score_threshold: -10,
        exclude_tokens: [],
...
async function main() {
  const existingBatchIds = getHeroiconsBatchIds(await listSemanticAutomationBatchIds());
  ...
  const remainingCount = heroiconsTotal - priorWorklistIds.size;

  if (remainingCount <= 0) {
    console.log('build-next-heroicons-batch-selection: Heroicons is already fully staged.');
    return;
  }
  ...
  for (const phase of phaseConfigs) {
    ...
    if (
      artifacts.summary.selected_count >= phase.config.target_min ||
      (phase.phaseId === 'full_sweep' && artifacts.summary.selected_count > 0)
    ) {
      selectedPhase = phase;
  ...
  if (!selectedPhase || !selectedArtifacts) {
    throw new Error(`Could not build a valid Heroicons selection for ${nextBatchId}`);
  }
  ...
  console.log(
    `build-next-heroicons-batch-selection: batch=${nextBatchId} | phase=${selectedPhase.phaseId} | selected=${selectedArtifacts.summary.selected_count} | remaining=${remainingCount}`
  );
}
