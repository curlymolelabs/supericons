...
const libraryDir = path.join(automationRoot, 'bootstrap');
const generatedDir = path.join(repoRoot, 'data', 'si-registry', 'generated');

const BATCH_ID = process.argv[2] || 'bootstrap-visual-review-batch-01';
const SOURCE_BATCH_ID = process.argv[3] || 'bootstrap-batch-01';
...
function buildFallbackReviewDecision(record) {
  ...
  return buildGenericDraftDecision(
      'This Bootstrap shape still needs stronger nearby product wording before it is safe to approve.',
      0.74
    );
}
...
const batchRecords = selectedIds.map((iconId) => {
  ...
  if (!worklistItem || !candidateRecord || !queueItem) {
    throw new Error(`Missing Bootstrap staged data for ${iconId}`);
  }

  if (queueItem.queue_outcome !== 'needs_visual_review') {
    throw new Error(`Selected icon is not in the Bootstrap visual-review queue: ${iconId}`);
  }

  const sourceIcon = iconIndexById.get(candidateRecord.source_asset_name);
  if (!sourceIcon?.svg) {
    throw new Error(`Missing Bootstrap SVG payload for ${iconId}`);
  }
...
for (const reviewedRecord of reviewedRecords) {
  ...
  throw new Error(`Unsupported Bootstrap review outcome for ${reviewedRecord.icon_id}`);
}
...
const batchData = {
  ...
  purpose: `Resolve the visually ambiguous Bootstrap icons from ${SOURCE_BATCH_ID}.`,
...
This batch resolves the visually ambiguous Bootstrap icons from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-bootstrap-visual-review-batch: batch=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
