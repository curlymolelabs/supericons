...
if (!batchId) {
  throw new Error('Usage: node scripts/build-mingcute-contact-sheet.mjs <batch-id>');
}

const batchPath = path.join(repoRoot, 'data', 'si-registry', 'automation', 'mingcute', `${batchId}.json`);
...
function renderCard(record, index) {
  ...
  const payload = icon.renderable_icon_payload?.svg || '';
  ...
  return `
    <g transform="translate(${x}, ${y})">
      ...
      <svg x="24" y="24" width="${ICON_BOX_SIZE}" height="${ICON_BOX_SIZE}" viewBox="${iconViewBox}" style="color:#162033">
        ${iconMarkup}
      </svg>
      <text x="140" y="48" font-size="14" font-weight="700" fill="#162033">${escapeHtml(label)}</text>
      <text x="140" y="70" font-size="12" fill="#4d5a70">${escapeHtml(record.icon_id)}</text>
      <text x="24" y="146" font-size="12" font-weight="700" fill="#2456d3">${escapeHtml(record.family_key)}</text>
      <text x="24" y="168" font-size="12" fill="#4d5a70">Queue: ${escapeHtml(record.queue_outcome)}</text>
      <text x="24" y="188" font-size="12" fill="#4d5a70">Visual: ${escapeHtml(icon.visual_payload_status)}</text>
    </g>
  `;
}
...
console.log(`build-mingcute-contact-sheet: wrote ${path.relative(repoRoot, svgOutputPath)}`);
console.log(`build-mingcute-contact-sheet: wrote ${path.relative(repoRoot, pngOutputPath)}`);
