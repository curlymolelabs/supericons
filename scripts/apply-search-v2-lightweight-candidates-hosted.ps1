param(
    [switch]$ExecuteApprovedLatencyGateA
)

$ErrorActionPreference = 'Stop'

if (-not $ExecuteApprovedLatencyGateA) {
    throw 'This runner is limited to an owner-approved Search v2 latency Gate A. Pass -ExecuteApprovedLatencyGateA to continue.'
}

$repoRoot = Split-Path -Parent $PSScriptRoot
$migrationVersion = '20260713150000'
$migrationName = "${migrationVersion}_search_v2_lightweight_candidates.sql"
...
$expectedMigrationHash = '8ad558920ae3565bd26fe3706a1ba8ef0e8c3b2ac9ddafce9f7b15e995ede42e'
...
$script:databaseUrl = "${poolerUrl}?sslmode=require&application_name=supericons_latency_gate_a"
...
try {
    ...
    Invoke-PsqlFile -ContainerPath '/checks/search-v2-lightweight-candidates-hosted-preflight.sql'
    Invoke-PsqlFile -ContainerPath "/migrations/$migrationName" -SingleTransaction
    Invoke-PsqlFile -ContainerPath '/checks/search-v2-lightweight-candidates-hosted-postflight.sql'
    ...
    Write-Output "Search v2 lightweight candidate migration $migrationVersion completed."
}
finally {
    $plainPassword = $null
