...
for (const [batchId, batchDecision] of Object.entries(decisions.batches || {})) {
  ...
  for (const entry of approveEntries) {
    ...
    if (seenApproved.has(entry.icon_id)) {
      throw new Error(`Duplicate approved Simple Icons id: ${entry.icon_id}`);
    }
  ...
  for (const entry of holdEntries) {
    ...
    if (seenHold.has(entry.icon_id)) {
      throw new Error(`Duplicate Simple Icons hold id: ${entry.icon_id}`);
    }
    editorHoldQueue.push(buildEditorHoldQueueRecord(reviewedRecord, batchId, entry.note || 'Needs tighter brand framing before approval.'));
  ...
  for (const entry of draftEntries) {
    ...
    if (seenDraft.has(entry.icon_id)) {
      throw new Error(`Duplicate Simple Icons draft id: ${entry.icon_id}`);
    }
    draftRecords.push({
      batch_id: batchId,
...
const approvalSummary = {
  ...
  approved_record_path: 'data/si-registry/automation/simpleicons/approved-records.json',
  editor_hold_queue_path: 'data/si-registry/automation/simpleicons/editor-hold-queue.json',
};
...
await writeJson(path.join(generatedDir, 'simpleicons-approval-summary.json'), approvalSummary);

console.log(`build-simpleicons-approved-records: approved=${approvedRecords.length}, hold=${editorHoldQueue.length}, draft=${draftRecords.length}`);
