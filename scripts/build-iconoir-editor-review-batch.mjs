...
const BATCH_ID = process.argv[2] || 'iconoir-editor-review-batch-01';
...
const reviewedRecords = selectedIds.map((iconId) => {
  ...
  if (!candidateRecord) {
      throw new Error(`Missing Iconoir candidate record for ${iconId}`);
  }
...
const batchData = {
  ...
  purpose: 'Approve the high-confidence Iconoir editor-review queue from the current Iconoir automation batch.',
...
This batch clears the high-confidence Iconoir editor-review queue from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-iconoir-editor-review-batch: reviewed=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
