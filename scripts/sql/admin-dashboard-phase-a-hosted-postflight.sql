...
begin
  ...
  execute 'select (select count(*) from public.admin_rollup_overview) + (select count(*) from public.admin_rollup_queries)'
    into rollup_count;
  if rollup_count <> 0 then
    raise exception 'Phase A rollup tables were not empty after migration: %', rollup_count;
  end if;
end $$;
