...
const BATCH_ID = process.argv[2] || 'bootstrap-editor-review-batch-01';
...
const reviewedRecords = selectedIds.map((iconId) => {
  ...
  if (!candidateRecord) {
      throw new Error(`Missing Bootstrap candidate record for ${iconId}`);
  }
...
const batchData = {
  ...
  purpose: 'Approve the high-confidence Bootstrap editor-review queue from the current Bootstrap automation batch.',
...
This batch clears the high-confidence Bootstrap editor-review queue from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-bootstrap-editor-review-batch: reviewed=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
