...
const BATCH_ID = process.argv[2] || 'tabler-editor-review-batch-01';
...
const reviewedRecords = selectedIds.map((iconId) => {
  ...
  if (!candidateRecord) {
    throw new Error(`Missing Tabler candidate record for ${iconId}`);
  }
...
const batchData = {
  ...
  purpose: 'Approve the high-confidence Tabler editor-review queue from the current Tabler automation batch.',
...
This batch clears the high-confidence Tabler editor-review queue from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-tabler-editor-review-batch: reviewed=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
