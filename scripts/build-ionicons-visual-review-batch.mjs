...
const libraryDir = path.join(automationRoot, 'ionicons');
const generatedDir = path.join(repoRoot, 'data', 'si-registry', 'generated');

const BATCH_ID = process.argv[2] || 'ionicons-visual-review-batch-01';
const SOURCE_BATCH_ID = process.argv[3] || 'ionicons-batch-01';
...
function buildFallbackReviewDecision(record) {
  ...
  return buildGenericDraftDecision(
      'This Ionicons shape still needs stronger nearby product wording before it is safe to approve.',
      0.74
    );
}
...
const batchRecords = selectedIds.map((iconId) => {
  ...
  if (!worklistItem || !candidateRecord || !queueItem) {
    throw new Error(`Missing Ionicons staged data for ${iconId}`);
  }

  if (queueItem.queue_outcome !== 'needs_visual_review') {
    throw new Error(`Selected icon is not in the Ionicons visual-review queue: ${iconId}`);
  }

  const sourceIcon = iconIndexById.get(candidateRecord.source_asset_name);
  if (!sourceIcon?.svg) {
    throw new Error(`Missing Ionicons SVG payload for ${iconId}`);
  }
...
for (const reviewedRecord of reviewedRecords) {
  ...
  throw new Error(`Unsupported Ionicons review outcome for ${reviewedRecord.icon_id}`);
}
...
const batchData = {
  ...
  purpose: `Resolve the visually ambiguous Ionicons icons from ${SOURCE_BATCH_ID}.`,
...
This batch resolves the visually ambiguous Ionicons icons from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-ionicons-visual-review-batch: batch=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
