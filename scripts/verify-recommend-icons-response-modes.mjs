...
const supericonsLogoResult = await recommendFor({
  task: 'Recommend brand logos for an AI coding agent landing page using the Supericons logo library.',
  library: 'si',
  slots: ['OpenAI Codex logo', 'Lovable logo', 'Kickbacks.ai logo', 'xAI Grok logo'],
});
const expectedSupericonsLogos = new Map([
  ['OpenAI Codex logo', 'openai-codex-app'],
  ['Lovable logo', 'lovable'],
  ['Kickbacks.ai logo', 'kickbacks-ai'],
  ['xAI Grok logo', 'x-ai'],
]);
assert(supericonsLogoResult.all_slots_resolved === true, 'Supericons logo recommendations should resolve every requested logo slot.');
assert(supericonsLogoResult.low_confidence_slots.length === 0, 'Supericons logo recommendations should not be low confidence.');
for (const [slotName, expectedId] of expectedSupericonsLogos) {
  const slot = getSlot(supericonsLogoResult, slotName);
  assert(slot?.recommended?.library === 'si', `${slotName} should recommend from the si library.`);
  assert(slot?.recommended?.id === expectedId, `${slotName} should recommend ${expectedId}; got ${slot?.recommended?.id || 'none'}.`);
}
