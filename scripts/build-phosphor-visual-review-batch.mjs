...
const libraryDir = path.join(automationRoot, 'phosphor');
const generatedDir = path.join(repoRoot, 'data', 'si-registry', 'generated');

const BATCH_ID = process.argv[2] || 'phosphor-visual-review-batch-01';
const SOURCE_BATCH_ID = process.argv[3] || 'phosphor-batch-01';
...
function buildFallbackReviewDecision(record) {
  ...
  return buildGenericDraftDecision(
      'This Phosphor shape still needs stronger nearby product wording before it is safe to approve.',
      0.74
    );
}
...
const batchRecords = selectedIds.map((iconId) => {
  ...
  if (!worklistItem || !candidateRecord || !queueItem) {
    throw new Error(`Missing Phosphor staged data for ${iconId}`);
  }

  if (queueItem.queue_outcome !== 'needs_visual_review') {
    throw new Error(`Selected icon is not in the Phosphor visual-review queue: ${iconId}`);
  }

  const sourceIcon = iconIndexById.get(candidateRecord.source_asset_name);
  if (!sourceIcon?.svg) {
    throw new Error(`Missing Phosphor SVG payload for ${iconId}`);
  }
...
for (const reviewedRecord of reviewedRecords) {
  ...
  throw new Error(`Unsupported Phosphor review outcome for ${reviewedRecord.icon_id}`);
}
...
const batchData = {
  ...
  purpose: `Resolve the visually ambiguous Phosphor icons from ${SOURCE_BATCH_ID}.`,
...
This batch resolves the visually ambiguous Phosphor icons from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-phosphor-visual-review-batch: batch=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
