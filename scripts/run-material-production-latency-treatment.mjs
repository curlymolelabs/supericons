...
function markTreatmentDedupeKeys(value) {
  if (Array.isArray(value)) return value.map(markTreatmentDedupeKeys);
  if (!value || typeof value !== 'object') return value;
  const output = Object.fromEntries(
    Object.entries(value).map(([key, child]) => [key, markTreatmentDedupeKeys(child)]),
  );
  if (typeof output.dedupe_key === 'string') {
    output.dedupe_key = output.dedupe_key.replace(/^material-baseline:/, 'material-treatment:');
  }
  return output;
}
...
assert.equal(readArg('variant'), 'treatment', 'Material treatment measurements require treatment variant.');
...
assert.ok(
  ...
  'Material production treatment must target stable mcp-search.',
);
...
globalThis.fetch = async (input, init = {}) => {
  ...
  assert.equal(typeof init.body, 'string', 'Stable treatment request body must be JSON text.');
  const payload = JSON.parse(init.body);
  const productionPayload = markTreatmentDedupeKeys(productionizeMeasurementPayload(payload, {
    runId,
    requestSequence: sanitizedRequestCount,
  }));
  const encoded = JSON.stringify(productionPayload);
  assert.ok(!encoded.includes('mcp_beta'), 'Stable treatment request retained beta source.');
  assert.ok(!encoded.includes('"environment":"preview"'), 'Stable treatment request retained preview environment.');
  assert.ok(!encoded.includes('beta_cohort'), 'Stable treatment request retained beta cohort.');
  assert.ok(encoded.includes('material-treatment:'), 'Stable treatment request lacks treatment dedupe identity.');
...
assert.equal(sanitizedRequestCount, expectedRequestCount, 'Stable treatment request count changed.');

const outputPath = resolve(output);
const artifact = JSON.parse(readFileSync(outputPath, 'utf8'));
assert.equal(artifact.endpoint, ENDPOINT_NAME);
assert.equal(artifact.mode, mode);
assert.equal(artifact.variant, 'treatment');
...
console.log(JSON.stringify({
  ...
  measurement_phase: retainedArtifact.measurement_phase,
