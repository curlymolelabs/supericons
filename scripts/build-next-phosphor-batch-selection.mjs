...
function getPhosphorBatchIds(batchIds) {
  return batchIds.filter((batchId) => /^phosphor-batch-\d+$/i.test(batchId));
}
...
function getNextBatchId(batchIds) {
  const maxNumber = batchIds.reduce((currentMax, batchId) => {
    const match = batchId.match(/^phosphor-batch-(\d+)$/i);
    if (!match) return currentMax;
    return Math.max(currentMax, Number(match[1]));
  }, 0);
  return `phosphor-batch-${String(maxNumber + 1).padStart(2, '0')}`;
}
...
function buildPhaseConfigs(template, nextBatchId, targetConfig, excludeBatchIds) {
  ...
  return [
    {
      phaseId: 'high_signal',
      description: 'Continue the Phosphor rollout with the strongest remaining UI and product-control icons.',
      config: {
        ...common,
        score_threshold: 2,
    ...
    {
      phaseId: 'relaxed',
      description: 'Broaden Phosphor coverage while still avoiding the obviously decorative long-tail shapes.',
      config: {
        ...common,
        score_threshold: 0,
        exclude_tokens: [...(template.exclude_tokens || [])],
    ...
    {
      phaseId: 'full_sweep',
      description: 'Sweep the remaining Phosphor library so the rollout can fully close.',
      config: {
        ...common,
        score_threshold: -10,
        exclude_tokens: [],
...
async function main() {
  const existingBatchIds = getPhosphorBatchIds(await listSemanticAutomationBatchIds());
  ...
  const remainingCount = phosphorTotal - priorWorklistIds.size;

  if (remainingCount <= 0) {
    console.log('build-next-phosphor-batch-selection: Phosphor is already fully staged.');
    return;
  }
  ...
  if (!selectedPhase || !selectedArtifacts) {
    throw new Error(`Could not build a valid Phosphor selection for ${nextBatchId}`);
  }
  ...
  console.log(
    `build-next-phosphor-batch-selection: batch=${nextBatchId} | phase=${selectedPhase.phaseId} | selected=${selectedArtifacts.summary.selected_count} | remaining=${remainingCount}`
  );
}
