...
const attributionProbePath = 'scripts/probe-material-engine-attribution.mjs';
const attributionProbeVerifierPath = 'scripts/verify-material-engine-attribution.mjs';
...
const priorRespinAttemptPath = 'references/verification/material-railway-recovery-respin-attempt-2026-07-15.md';
const priorRespinEvidencePaths = [
  'references/verification/material-railway-recovery-respin-legacy-preflight-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-stability-preflight-attempt-1-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-stability-preflight-attempt-2-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-stability-preflight-attempt-3-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-material-gate-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-engine-attempt-1-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-control-attempt-1-2026-07-15.json',
  'references/verification/material-railway-recovery-respin-rollback-2026-07-15.json',
];
...
assert.deepEqual(fields, {
  ...
  attribution_probe_sha256: fields.attribution_probe_sha256,
  attribution_probe_verifier_sha256: fields.attribution_probe_verifier_sha256,
  ...
  prior_respin_attempt_sha256: fields.prior_respin_attempt_sha256,
  prior_respin_legacy_preflight_sha256: fields.prior_respin_legacy_preflight_sha256,
  prior_respin_stability_attempt_1_sha256: fields.prior_respin_stability_attempt_1_sha256,
  prior_respin_stability_attempt_2_sha256: fields.prior_respin_stability_attempt_2_sha256,
  prior_respin_stability_attempt_3_sha256: fields.prior_respin_stability_attempt_3_sha256,
  prior_respin_material_gate_sha256: fields.prior_respin_material_gate_sha256,
  prior_respin_engine_attempt_1_sha256: fields.prior_respin_engine_attempt_1_sha256,
  prior_respin_control_attempt_1_sha256: fields.prior_respin_control_attempt_1_sha256,
  prior_respin_rollback_sha256: fields.prior_respin_rollback_sha256,
  ...
  predeployment_id: '01453e0d-20e5-496c-a9da-40b135e173c4',
  ...
  attribution_overhead_budget_ms: '1000',
  attributed_engine_latency_cases: '3',
  ...
  engine_latency_failure_action: 'query_matched_direct_engine_attribution',
...
const hashFields = [
  ...
  'attribution_probe_sha256', 'attribution_probe_verifier_sha256',
  ...
  'prior_legacy_preflight_sha256', 'prior_stability_preflight_sha256',
  'prior_respin_attempt_sha256', 'prior_respin_legacy_preflight_sha256',
  'prior_respin_stability_attempt_1_sha256', 'prior_respin_stability_attempt_2_sha256',
  'prior_respin_stability_attempt_3_sha256', 'prior_respin_material_gate_sha256',
  'prior_respin_engine_attempt_1_sha256', 'prior_respin_control_attempt_1_sha256',
  'prior_respin_rollback_sha256', 'bundle_sha256',
];
...
for (const [path, field] of [
  ...
  [attributionProbePath, 'attribution_probe_sha256'],
  [attributionProbeVerifierPath, 'attribution_probe_verifier_sha256'],
  ...
  [priorRespinAttemptPath, 'prior_respin_attempt_sha256'],
  [priorRespinEvidencePaths[0], 'prior_respin_legacy_preflight_sha256'],
  [priorRespinEvidencePaths[1], 'prior_respin_stability_attempt_1_sha256'],
  [priorRespinEvidencePaths[2], 'prior_respin_stability_attempt_2_sha256'],
  [priorRespinEvidencePaths[3], 'prior_respin_stability_attempt_3_sha256'],
  [priorRespinEvidencePaths[4], 'prior_respin_material_gate_sha256'],
  [priorRespinEvidencePaths[5], 'prior_respin_engine_attempt_1_sha256'],
  [priorRespinEvidencePaths[6], 'prior_respin_control_attempt_1_sha256'],
  [priorRespinEvidencePaths[7], 'prior_respin_rollback_sha256'],
]) {
...
assert.match(runner, /\$AttributionOverheadBudgetMilliseconds = 1000/);
...
assert.match(runner, /candidate_engine_correctness_or_local_latency_failure_attempt_/);
assert.match(runner, /candidate_engine_overhead_exceeded_attempt_/);
assert.match(runner, /candidate_engine_attribution_invalid_evidence_attempt_/);
assert.match(runner, /dependency_unresolved_after_retry_budget/);
assert.match(runner, /material-railway-recovery-attribution-stability-preflight-attempt-\$attempt-2026-07-15\.json/);
assert.match(runner, /material-railway-recovery-attribution-attempt-\$attempt-2026-07-15\.json/);
...
assert.match(runner, /'--latency-limit-ms', "\$HealthyProbeLatencyLimitMilliseconds"/,
  'The stability preflight must use the pinned 5,000 ms health threshold.');
...
assert.match(runner, /'--overhead-budget-ms', "\$AttributionOverheadBudgetMilliseconds"/,
  'Query-matched attribution must use the pinned 1,000 ms candidate overhead budget.');
assert.equal(
...
assert.match(runner, /Invoke-EngineAttribution/,
  'Engine latency failures must use query-matched direct attribution.');
assert.equal(runner.includes('candidate_engine_gate_failed_with_healthy_control_attempt_'), false,
  'The mismatched generic control policy must not survive the respin.');
assert.ok(
...
assert.match(recoveryGate, /summary\.status = 'latency_failed'/);
assert.match(recoveryGate, /caseId: `all_mode_\$\{query\}`/);
assert.match(recoveryGate, /caseId: 'lucide_strict_calendar'/);
assert.match(recoveryGate, /path: 'candidate_local'/);

const attributionProbe = normalizedText(readFileSync(attributionProbePath, 'utf8'));
for (const required of [
  ...
  'failure.through_candidate_ms <= comparison.allowed_candidate_ms',
]) {
  assert.ok(attributionProbe.includes(required), `Attribution probe is missing: ${required}`);
}
...
execFileSync(process.execPath, [attributionProbeVerifierPath], { stdio: 'inherit' });
...
console.log(JSON.stringify({
  ...
  postdeploy_gates: {
    ...
    query_matched_attribution_cases: Number(fields.attributed_engine_latency_cases),
    attribution_overhead_budget_ms: Number(fields.attribution_overhead_budget_ms),
  },
  mutations: {
