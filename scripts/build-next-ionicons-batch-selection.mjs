...
const TEMPLATE_PATH = path.join(automationRoot, 'ionicons-template.json');
...
function getIoniconsBatchIds(batchIds) {
  return batchIds.filter((batchId) => /^ionicons-batch-\d+$/i.test(batchId));
}
...
function getNextBatchId(batchIds) {
  const maxNumber = batchIds.reduce((currentMax, batchId) => {
    const match = batchId.match(/^ionicons-batch-(\d+)$/i);
    if (!match) return currentMax;
    return Math.max(currentMax, Number(match[1]));
  }, 0);
  return `ionicons-batch-${String(maxNumber + 1).padStart(2, '0')}`;
}
...
function buildPhaseConfigs(template, nextBatchId, targetConfig, excludeBatchIds) {
  ...
  return [
    {
      phaseId: 'high_signal',
      description: 'Continue the Ionicons rollout with the strongest remaining UI and product-control icons.',
      config: {
        ...common,
        score_threshold: 2,
    ...
    {
      phaseId: 'relaxed',
      description: 'Broaden Ionicons coverage while still avoiding the obviously decorative long-tail shapes.',
      config: {
        ...common,
        score_threshold: 0,
        exclude_tokens: [...(template.exclude_tokens || [])],
    ...
    {
      phaseId: 'full_sweep',
      description: 'Sweep the remaining Ionicons library so the rollout can fully close.',
      config: {
        ...common,
        score_threshold: -10,
        exclude_tokens: [],
...
async function main() {
  const existingBatchIds = getIoniconsBatchIds(await listSemanticAutomationBatchIds());
  ...
  const remainingCount = ioniconsTotal - priorWorklistIds.size;

  if (remainingCount <= 0) {
    console.log('build-next-ionicons-batch-selection: Ionicons is already fully staged.');
    return;
  }
  ...
  if (!selectedPhase || !selectedArtifacts) {
    throw new Error(`Could not build a valid Ionicons selection for ${nextBatchId}`);
  }
  ...
  console.log(
    `build-next-ionicons-batch-selection: batch=${nextBatchId} | phase=${selectedPhase.phaseId} | selected=${selectedArtifacts.summary.selected_count} | remaining=${remainingCount}`
  );
}
