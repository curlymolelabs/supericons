...
const libraryDir = path.join(automationRoot, 'iconoir');
const generatedDir = path.join(repoRoot, 'data', 'si-registry', 'generated');

const BATCH_ID = process.argv[2] || 'iconoir-visual-review-batch-01';
const SOURCE_BATCH_ID = process.argv[3] || 'iconoir-batch-01';
...
function buildFallbackReviewDecision(record) {
  ...
  return buildGenericDraftDecision(
      'This Iconoir shape still needs stronger nearby product wording before it is safe to approve.',
      0.74
    );
}
...
const batchRecords = selectedIds.map((iconId) => {
  ...
  if (!worklistItem || !candidateRecord || !queueItem) {
    throw new Error(`Missing Iconoir staged data for ${iconId}`);
  }

  if (queueItem.queue_outcome !== 'needs_visual_review') {
    throw new Error(`Selected icon is not in the Iconoir visual-review queue: ${iconId}`);
  }

  const sourceIcon = iconIndexById.get(candidateRecord.source_asset_name);
  if (!sourceIcon?.svg) {
    throw new Error(`Missing Iconoir SVG payload for ${iconId}`);
  }
...
for (const reviewedRecord of reviewedRecords) {
  ...
  throw new Error(`Unsupported Iconoir review outcome for ${reviewedRecord.icon_id}`);
}
...
const batchData = {
  ...
  purpose: `Resolve the visually ambiguous Iconoir icons from ${SOURCE_BATCH_ID}.`,
...
This batch resolves the visually ambiguous Iconoir icons from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-iconoir-visual-review-batch: batch=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
