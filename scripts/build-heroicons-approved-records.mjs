...
for (const [batchId, batchDecision] of Object.entries(decisions.batches || {})) {
  ...
  for (const entry of approveEntries) {
    ...
    if (!reviewedRecord) throw new Error(`Missing reviewed Heroicons record for approved icon ${entry.icon_id} in ${batchId}`);
    ...
    if (seenApproved.has(entry.icon_id)) throw new Error(`Duplicate approved Heroicons icon id: ${entry.icon_id}`);
  ...
  for (const entry of holdEntries) {
    const reviewedRecord = reviewedById.get(entry.icon_id);
    if (!reviewedRecord) throw new Error(`Missing reviewed Heroicons record for hold icon ${entry.icon_id} in ${batchId}`);
    if (seenHold.has(entry.icon_id)) throw new Error(`Duplicate Heroicons hold icon id: ${entry.icon_id}`);
    editorHoldQueue.push(buildEditorHoldQueueRecord(reviewedRecord, batchId, entry.note || 'Needs tighter Heroicons editor framing before approval.'));
    seenHold.add(entry.icon_id);
  }

  for (const entry of draftEntries) {
    const reviewedRecord = reviewedById.get(entry.icon_id);
    if (!reviewedRecord) throw new Error(`Missing reviewed Heroicons record for draft icon ${entry.icon_id} in ${batchId}`);
    if (seenDraft.has(entry.icon_id)) throw new Error(`Duplicate Heroicons draft icon id: ${entry.icon_id}`);
    draftRecords.push({
      batch_id: batchId,
...
const approvalSummary = {
  ...
  approved_record_path: 'data/si-registry/automation/heroicons/approved-records.json',
  editor_hold_queue_path: 'data/si-registry/automation/heroicons/editor-hold-queue.json',
};
...
await writeJson(path.join(generatedDir, 'heroicons-approval-summary.json'), approvalSummary);

console.log(
  `build-heroicons-approved-records: approved=${approvedRecords.length}, hold=${editorHoldQueue.length}, draft=${draftRecords.length}`
);
