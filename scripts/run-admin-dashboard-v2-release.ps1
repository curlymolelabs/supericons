...
$FingerprintSource = Join-Path $Root 'references/verification/admin-dashboard-v2-release-fingerprint-2026-07-17.txt'
$LinkedProjectPath = Join-Path $Root 'supabase/.temp/linked-project.json'
$Workspace = Join-Path $Root 'tmp/admin-dashboard-v2-release'
$PreInventoryEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-pre-inventory-2026-07-17.json'
$DatabaseEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-database-preflight-2026-07-17.json'
$RailwayEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-railway-health-2026-07-17.json'
$SearchEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-search-health-2026-07-17.json'
$LegacyPreflightEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-legacy-preflight-2026-07-17.json'
$CandidateInventoryEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-candidate-inventory-2026-07-17.json'
$V2LiveEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-live-2026-07-17.json'
$PhaseALiveEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-phase-a-regression-2026-07-17.json'
$CompletionEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-completion-2026-07-17.json'
$RollbackInventoryEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-rollback-inventory-2026-07-17.json'
$RollbackEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-rollback-2026-07-17.json'
$RollbackFailureEvidence = Join-Path $Root 'references/verification/admin-dashboard-v2-release-rollback-failure-2026-07-17.json'
...
function Invoke-Rollback {
  ...
  try {
    ...
    $rollbackInventory = Invoke-Inventory `
      -Output 'references/verification/admin-dashboard-v2-release-rollback-inventory-2026-07-17.json'
    ...
    $rollbackLive = Invoke-NodeEvidenceGate `
      ...
      -Arguments @(
        '--admin-url', $AdminUrl,
        '--mode', 'legacy',
        '--output', 'tmp/admin-dashboard-v2-release/rollback-live.json'
      ) `
      -Output 'tmp/admin-dashboard-v2-release/rollback-live.json'
    Write-JsonEvidence -Path $RollbackEvidence -Value ([ordered]@{
      artifact = 'admin_dashboard_v2_release_rollback'
    ...
    Write-JsonEvidence -Path $RollbackFailureEvidence -Value ([ordered]@{
      artifact = 'admin_dashboard_v2_release_rollback_failure'
      release_fingerprint = $script:ReleaseFingerprint
...
try {
  ...
  $preInventory = Invoke-Inventory `
    -Output 'references/verification/admin-dashboard-v2-release-pre-inventory-2026-07-17.json'
  ...
  $database = Invoke-NodeEvidenceGate `
    ...
    -Arguments @(
      ...
      '--output', 'references/verification/admin-dashboard-v2-release-database-preflight-2026-07-17.json'
    ) `
    -Output 'references/verification/admin-dashboard-v2-release-database-preflight-2026-07-17.json'
  ...
  $null = Invoke-QuietCheckedCommand -FilePath 'node' -Arguments @(
    ...
    '--allow-active',
    '--output', 'references/verification/admin-dashboard-v2-release-railway-health-2026-07-17.json'
  )
  ...
  $null = Invoke-QuietCheckedCommand -FilePath 'node' -Arguments @(
    ...
    '--output', 'references/verification/admin-dashboard-v2-release-search-health-2026-07-17.json',
    '--warmup-count', '1',
    '--measured-count', '2',
  ...
  $legacyPreflight = Invoke-NodeEvidenceGate `
    ...
    -Arguments @(
      ...
      '--output', 'references/verification/admin-dashboard-v2-release-legacy-preflight-2026-07-17.json'
    ) `
    -Output 'references/verification/admin-dashboard-v2-release-legacy-preflight-2026-07-17.json'
  ...
  $candidateInventory = Invoke-Inventory `
    -Output 'references/verification/admin-dashboard-v2-release-candidate-inventory-2026-07-17.json'
  ...
  $v2Live = Invoke-NodeEvidenceGate `
    ...
    -Arguments @(
      '--admin-url', $AdminUrl,
      '--release-fingerprint', $script:ReleaseFingerprint,
      '--output', 'references/verification/admin-dashboard-v2-release-live-2026-07-17.json'
    ) `
    -Output 'references/verification/admin-dashboard-v2-release-live-2026-07-17.json'
  ...
  $phaseALive = Invoke-NodeEvidenceGate `
    ...
    -Arguments @(
      ...
      '--output', 'references/verification/admin-dashboard-v2-release-phase-a-regression-2026-07-17.json'
    ) `
    -Output 'references/verification/admin-dashboard-v2-release-phase-a-regression-2026-07-17.json'
  ...
  Write-JsonEvidence -Path $CompletionEvidence -Value ([ordered]@{
    artifact = 'admin_dashboard_v2_release_completion'
    release_fingerprint = $script:ReleaseFingerprint
  ...
  Write-Host "Admin dashboard V2 API release completed at function version $($candidate.version)."
}
catch {
