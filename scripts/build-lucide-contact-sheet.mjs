...
if (!batchId) {
  throw new Error('Usage: node scripts/build-lucide-contact-sheet.mjs <batch-id>');
}

const batchPath = path.join(repoRoot, 'data', 'si-registry', 'automation', 'lucide', `${batchId}.json`);
...
function renderCard(record, index) {
  ...
  return `
    <g transform="translate(${x}, ${y})">
      ...
      <svg x="24" y="24" width="${ICON_BOX_SIZE}" height="${ICON_BOX_SIZE}" viewBox="${getViewBox(payload)}" style="color:#162033">
        ${getInnerSvg(payload)}
      </svg>
...
console.log(`build-lucide-contact-sheet: wrote ${path.relative(repoRoot, svgOutputPath)}`);
console.log(`build-lucide-contact-sheet: wrote ${path.relative(repoRoot, pngOutputPath)}`);
