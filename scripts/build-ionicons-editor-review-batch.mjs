...
const BATCH_ID = process.argv[2] || 'ionicons-editor-review-batch-01';
...
const reviewedRecords = selectedIds.map((iconId) => {
  ...
  if (!candidateRecord) {
      throw new Error(`Missing Ionicons candidate record for ${iconId}`);
  }
...
const batchData = {
  ...
  purpose: 'Approve the high-confidence Ionicons editor-review queue from the current Ionicons automation batch.',
...
This batch clears the high-confidence Ionicons editor-review queue from ${SOURCE_BATCH_ID}.
...
console.log(
  `build-ionicons-editor-review-batch: reviewed=${reviewedRecords.length}, approved=${approveForImport.length}, hold=${holdForEditorReview.length}, draft=${keepAsReviewedDraft.length}`
);
