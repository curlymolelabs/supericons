...
async function getIconoirBatchIds() {
  const entries = await fs.readdir(automationRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && /^iconoir-batch-\d+-selection\.json$/i.test(entry.name))
    .map((entry) => entry.name.replace(/-selection\.json$/i, ''))
    .sort((left, right) => left.localeCompare(right, undefined, { numeric: true }));
}

function buildEditorBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^iconoir-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Iconoir editor batch id from ${sourceBatchId}`);
  return `iconoir-editor-review-batch-${match[1].padStart(2, '0')}`;
}

function buildVisualBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^iconoir-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Iconoir visual batch id from ${sourceBatchId}`);
  return `iconoir-visual-review-batch-${match[1].padStart(2, '0')}`;
}

async function getResolvedIds() {
  const decisions = await readJsonOrDefault(path.join(automationRoot, 'iconoir', 'promotion-decisions.json'), {
...
async function getStagedIconoirCount(batchIds) {
  let count = 0;
...
async function processExistingBatches(batchIds) {
  ...
  for (const batchId of batchIds) {
    ...
    if (readyIds.length > 0) {
      runNodeScript('build-iconoir-editor-review-batch.mjs', buildEditorBatchId(batchId), batchId);
      didWork = true;
    }
    if (visualIds.length > 0) {
      runNodeScript('build-iconoir-visual-review-batch.mjs', buildVisualBatchId(batchId), batchId);
      didWork = true;
    }
  }

  if (didWork) {
    runNodeScript('build-iconoir-approved-records.mjs');
  }
...
async function stageNextBatch() {
  const beforeBatchIds = await getIconoirBatchIds();
  const beforeSet = new Set(beforeBatchIds);
  runNodeScript('build-next-iconoir-batch-selection.mjs');
  const afterBatchIds = await getIconoirBatchIds();
...
async function main() {
  const iconoirTotal = (publicIconIndex.icons || []).filter((icon) => icon.lib === 'iconoir').length;
  let iterations = 0;

  while (iterations < 400) {
    iterations += 1;
    const batchIds = await getIconoirBatchIds();
    const processed = await processExistingBatches(batchIds);
    if (processed) {
      continue;
    }

    const stagedCount = await getStagedIconoirCount(batchIds);
    if (stagedCount >= iconoirTotal) {
      break;
    }
  ...
  runNodeScript('verify-iconoir-approved-records.mjs');
  ...
  console.log(
    `run-iconoir-library-rollout: approved=${approvalSummary.total_approved_records} | hold=${approvalSummary.total_editor_hold_records} | draft=${approvalSummary.total_reviewed_drafts} | public_free=${registrySummary.publicRecordCount}`
  );
}
