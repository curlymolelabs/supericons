...
async function getPhosphorBatchIds() {
  const entries = await fs.readdir(automationRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && /^phosphor-batch-\d+-selection\.json$/i.test(entry.name))
    .map((entry) => entry.name.replace(/-selection\.json$/i, ''))
    .sort((left, right) => left.localeCompare(right, undefined, { numeric: true }));
}

function buildEditorBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^phosphor-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Phosphor editor batch id from ${sourceBatchId}`);
  return `phosphor-editor-review-batch-${match[1].padStart(2, '0')}`;
}

function buildVisualBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^phosphor-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Phosphor visual batch id from ${sourceBatchId}`);
  return `phosphor-visual-review-batch-${match[1].padStart(2, '0')}`;
}

async function getResolvedIds() {
  const decisions = await readJsonOrDefault(path.join(automationRoot, 'phosphor', 'promotion-decisions.json'), {
...
async function getStagedPhosphorCount(batchIds) {
  let count = 0;
...
async function processExistingBatches(batchIds) {
  ...
  for (const batchId of batchIds) {
    ...
    if (readyIds.length > 0) {
      runNodeScript('build-phosphor-editor-review-batch.mjs', buildEditorBatchId(batchId), batchId);
      didWork = true;
    }
    if (visualIds.length > 0) {
      runNodeScript('build-phosphor-visual-review-batch.mjs', buildVisualBatchId(batchId), batchId);
      didWork = true;
    }
  }

  if (didWork) {
    runNodeScript('build-phosphor-approved-records.mjs');
  }
...
async function stageNextBatch() {
  const beforeBatchIds = await getPhosphorBatchIds();
  const beforeSet = new Set(beforeBatchIds);
  runNodeScript('build-next-phosphor-batch-selection.mjs');
  const afterBatchIds = await getPhosphorBatchIds();
...
async function main() {
  const phosphorTotal = (publicIconIndex.icons || []).filter((icon) => icon.lib === 'phosphor').length;
  let iterations = 0;

  while (iterations < 400) {
    iterations += 1;
    const batchIds = await getPhosphorBatchIds();
    const processed = await processExistingBatches(batchIds);
    if (processed) {
      continue;
    }

    const stagedCount = await getStagedPhosphorCount(batchIds);
    if (stagedCount >= phosphorTotal) {
      break;
    }
  ...
  runNodeScript('verify-phosphor-approved-records.mjs');
  ...
  console.log(
    `run-phosphor-library-rollout: approved=${approvalSummary.total_approved_records} | hold=${approvalSummary.total_editor_hold_records} | draft=${approvalSummary.total_reviewed_drafts} | public_free=${registrySummary.publicRecordCount}`
  );
}
