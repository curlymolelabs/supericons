...
async function getHeroiconsBatchIds() {
  const entries = await fs.readdir(automationRoot, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && /^heroicons-batch-\d+-selection\.json$/i.test(entry.name))
    .map((entry) => entry.name.replace(/-selection\.json$/i, ''))
    .sort((left, right) => left.localeCompare(right, undefined, { numeric: true }));
}

function buildEditorBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^heroicons-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Heroicons editor batch id from ${sourceBatchId}`);
  return `heroicons-editor-review-batch-${match[1].padStart(2, '0')}`;
}

function buildVisualBatchId(sourceBatchId) {
  const match = sourceBatchId.match(/^heroicons-batch-(\d+)$/i);
  if (!match) throw new Error(`Cannot derive Heroicons visual batch id from ${sourceBatchId}`);
  return `heroicons-visual-review-batch-${match[1].padStart(2, '0')}`;
}

async function getResolvedIds() {
  const decisions = await readJsonOrDefault(path.join(automationRoot, 'heroicons', 'promotion-decisions.json'), {
...
async function getStagedHeroiconsCount(batchIds) {
  let count = 0;
...
async function processExistingBatches(batchIds) {
  ...
  for (const batchId of batchIds) {
    ...
    if (readyIds.length > 0) {
      runNodeScript('build-heroicons-editor-review-batch.mjs', buildEditorBatchId(batchId), batchId);
      didWork = true;
    }
    if (visualIds.length > 0) {
      runNodeScript('build-heroicons-visual-review-batch.mjs', buildVisualBatchId(batchId), batchId);
      didWork = true;
    }
  }

  if (didWork) {
    runNodeScript('build-heroicons-approved-records.mjs');
  }
...
async function stageNextBatch() {
  const beforeBatchIds = await getHeroiconsBatchIds();
  const beforeSet = new Set(beforeBatchIds);
  runNodeScript('build-next-heroicons-batch-selection.mjs');
  const afterBatchIds = await getHeroiconsBatchIds();
...
async function main() {
  const heroiconsTotal = (publicIconIndex.icons || []).filter((icon) => icon.lib === 'heroicons').length;
  let iterations = 0;

  while (iterations < 400) {
    iterations += 1;
    const batchIds = await getHeroiconsBatchIds();
    const processed = await processExistingBatches(batchIds);
    if (processed) {
      continue;
    }

    const stagedCount = await getStagedHeroiconsCount(batchIds);
    if (stagedCount >= heroiconsTotal) {
      break;
    }
  ...
  runNodeScript('verify-heroicons-approved-records.mjs');
  ...
  console.log(
    `run-heroicons-library-rollout: approved=${approvalSummary.total_approved_records} | hold=${approvalSummary.total_editor_hold_records} | draft=${approvalSummary.total_reviewed_drafts} | public_free=${registrySummary.publicRecordCount}`
  );
}
